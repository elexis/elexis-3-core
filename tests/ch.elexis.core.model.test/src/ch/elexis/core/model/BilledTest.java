...
public class BilledTest extends AbstractTest {
	...
	public void create() {
		...
		billed.setAmount(1);
		// use components to set price, setPrice method is interpreted as manual price
		// change
		billed.setPoints(2);
		billed.setFactor(1);
	...
	private Money calculateTotalInvalidWay(IBilled billed) {
		return billed.getPrice().multiply(billed.getPrimaryScaleFactor()).multiply(billed.getSecondaryScaleFactor())
				.multiply(billed.getAmount());
	}
}
