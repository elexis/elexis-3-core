...
public class PrescriptionTest extends AbstractTest {
	...
	public void createFindDelete() {
		IPrescription prescription = new IPrescriptionBuilder(coreModelService, null, localArticle, patient, "1-0-0-1")
				.buildAndSave();
	...
	public void patientGetMedication() {
		...
		List<IPrescription> medication = patient.getMedication(Arrays.asList(EntryType.FIXED_MEDICATION));
		...
		medication = patient.getMedication(Arrays.asList(EntryType.FIXED_MEDICATION));
		assertNotNull(medication);
		assertTrue(medication.isEmpty());
