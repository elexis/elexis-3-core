...
public class Test_PersistentObject extends AbstractPersistentObjectTest {
	...
	public void testAddRemoveToJoint() throws InterruptedException {
		...
		assertNotSame(0L, lastUpdate);
		Thread.sleep(2);
		int retVal = impl.addToList(PersistentObjectImpl.FLD_JOINT_OTHER, opo.getId(), new String[0]);
	...
	public void testAddRemoveToList() throws InterruptedException {
		...
		assertNotSame(0L, lastUpdate);
		Thread.sleep(2);
		int retVal = impl.addToList(PersistentObjectImpl.FLD_LIST_OTHER, opo.getId(), new String[0]);
		assertNotSame(0, retVal);
	...
	public void testCreateOrModifyTable() {
		...
		// test the JdbcException thrown by the statement if FunFactor was still
		// VARCHAR(6)
