...
public class Test_StockService extends AbstractPersistentObjectTest {
	...
	// stockService.findStockEntryForArticleInStock(defaultStock,
	// artikel_A.storeToString());
	// assertEquals("STD", stockEntry_A_STD.getStock().getCode());
	// IStockEntry stockEntry_A_PUB = stockService
	// .findStockEntryForArticleInStock(stock_A_5_public,
	// artikel_A.storeToString());
	// assertEquals("A", stockEntry_A_PUB.getStock().getCode().trim()); // trim()
	// needed for postgres
	//
	// IStockEntry stockEntry = stockService
	// .findPreferredStockEntryForArticle(artikel_A.storeToString(),
	// stock_B_10_owner.getId());
	// assertEquals("PRV", stockEntry.getStock().getCode());
	// IStockEntry stockEntry_PUB =
	// stockService.findPreferredStockEntryForArticle(artikel_A.storeToString(),
	// null);
	// assertEquals("STD", stockEntry_PUB.getStock().getCode());
	//
	// stockService.unstoreArticleFromStock(stock_B_10_private,
	// artikel_A.storeToString());
	...
	// assertEquals(Availability.CRITICAL_STOCK,
	// stockService.getArticleAvailabilityForStock(stock,
	// artikel_C.storeToString()));
	...
	// IStockEntry prefSE =
	// stockService.findPreferredStockEntryForArticle(artikel_C.storeToString(),
	// null);
	...
	// IStockEntry se = CoreHub.getStockService().storeArticleInStock(stock,
	// art.storeToString());
	// se.setCurrentStock(12);
	// se.setMaximumStock(10);
	// se.setMinimumStock(5);
	// se = CoreHub.getStockService().storeArticleInStock(stock,
	// art2.storeToString());
	// se.setCurrentStock(12);
