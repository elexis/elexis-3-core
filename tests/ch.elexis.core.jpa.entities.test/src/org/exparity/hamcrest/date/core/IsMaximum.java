...
public class IsMaximum<T> extends TypeSafeDiagnosingMatcher<T> {
	...
	protected boolean matchesSafely(final T actual, final Description mismatchDesc) {
		...
		if (range.getMaximum() != actualValue) {
			mismatchDesc.appendText("date is the " + actualValue + " " + this.formatter.describe(this.datePart)
					+ " instead of " + range.getMinimum() + " " + this.formatter.describe(this.datePart));
			return false;
		} else {
			return true;
		}
