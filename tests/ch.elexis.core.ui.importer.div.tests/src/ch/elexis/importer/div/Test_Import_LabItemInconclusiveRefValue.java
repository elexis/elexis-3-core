...
public class Test_Import_LabItemInconclusiveRefValue {
	...
	public void testLabItemInconclusiveManualCreation() {
		...
		LabItem inconclusiveLabitem = new LabItem("EYECOL", "Eye Color", AllTests.testLab, LabItem.REFVAL_INCONCLUSIVE,
				LabItem.REFVAL_INCONCLUSIVE, "color", LabItemTyp.TEXT, "Test", "AB");
		LabResult eyeColor = new LabResult(pat, new TimeTool(), inconclusiveLabitem, "blue", "with greyish shades");
		...
		if ("de".equals(locale.getLanguage())) {
			...
			System.out.println(String.format("Skipping test for language %s produced %s", locale.getLanguage(),
					eyeColor.getPathologicDescription().getReference()));
		}
