...
public class ILabServiceTest extends AbstractServiceTest {
	...
	public static void before() {
		...
		_04_500 = new ILabItemBuilder(coreModelService, "TCHO-P", "TCHO-P", "3.88-5.66", "3.88-5.66", "mmol/l",
				LabItemTyp.NUMERIC, "04 Cholesterin", 500).origin(laboratory, "TCHO-P", true).buildAndSave();
		_04_501 = new ILabItemBuilder(coreModelService, "HDLC-P", "HDLC-P", "0.93-1.55", "1.16-1.78", "mmol/l",
				LabItemTyp.NUMERIC, "04 Cholesterin", 501).buildAndSave();
		_04_520 = new ILabItemBuilder(coreModelService, "TG-P", "TG-P", "0.56-1.68", "0.56-1.68", "mmol/l",
				LabItemTyp.NUMERIC, "04 Cholesterin", 520).buildAndSave();
		_04_510_LDL_CALCULATED = new ILabItemBuilder(coreModelService, "LDL", "LDL (calculated", "<4.0", "<4.0",
				"mmol/l", LabItemTyp.FORMULA, "04 Cholesterin", 510).build();
		...
		new ILabResultBuilder(coreModelService, _04_501, getPatient()).result("1.82").buildLabOrder("1").buildAndSave();
		new ILabResultBuilder(coreModelService, _04_520, getPatient()).result("1.85").buildLabOrder("1").buildAndSave();
	...
	public void evaluate() {
		...
		ILabResult ldlHldResult = new ILabResultBuilder(coreModelService, _04_510_LDL_CALCULATED, getPatient())
				.buildLabOrder("1").buildAndSave();
	...
	public void getLabResultsForPatientWithItemType() {
		ILabResult labResult = new ILabResultBuilder(coreModelService, _04_500, getPatient()).result("9.37")
				.buildLabOrder("1").buildAndSave();

		List<ILabResult> results = labService.getLabResultsForPatientWithItemType(getPatient(), LabItemTyp.NUMERIC,
				false);
		assertEquals(7, results.size());

		CoreModelServiceHolder.get().delete(labResult);
		results = labService.getLabResultsForPatientWithItemType(getPatient(), LabItemTyp.NUMERIC, false);
	...
	public void getLabOrdersInSameOrderIdGroup() {
		List<ILabOrder> labOrdersInSameOrderIdGroup = labService
				.getLabOrdersInSameOrderIdGroup(_04_500_labResult.getLabOrder(), false);
