...
public class IElexisEnvironmentServiceTest extends AbstractServiceTest {
	...
	private Optional<IElexisEnvironmentService> ee_service = OsgiServiceUtil
			.getService(IElexisEnvironmentService.class);
	private IMessageService messageService = OsgiServiceUtil.getService(IMessageService.class).get();
	private static IContextService contextService = OsgiServiceUtil.getService(IContextService.class).get();
	...
	public void rocketChatMessage_fromStation() {
		...
		TransientMessage message = messageService
				.prepare(getClass().getName() + "@" + contextService.getStationIdentifier(), "rocketchat:demouser");
