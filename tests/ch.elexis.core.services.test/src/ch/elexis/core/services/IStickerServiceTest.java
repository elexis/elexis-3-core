...
public class IStickerServiceTest extends AbstractServiceTest {
	...
	public void addSticker() {
		Optional<IPatient> patient = coreModelService.load(TestEntities.PATIENT_MALE_ID, IPatient.class);
	...
	//
	// Sticker sticker = new StickerService.StickerBuilder("Sticker 2", "123456",
	// "123456", ElexisTypeMap.TYPE_PATIENT)
	// .buildAndSave();
	//
	// assertEquals(1, StickerService.findAll(Sticker.class, false).size());
	// assertEquals("Sticker 2", sticker.getName());
	// assertEquals(1, sticker.getStickerClassLinks().size());
	// assertEquals(ElexisTypeMap.TYPE_PATIENT,
	// sticker.getStickerClassLinks().iterator().next().getObjclass());
	// Optional<Kontakt> findById =
	// KontaktService.load(TestEntities.PATIENT_MALE_ID);
	//
	// List<Sticker> findStickersOnObject =
	// StickerService.findStickersOnObject(findById.get());
	// assertEquals(0, findStickersOnObject.size());
	//
	// boolean stickerApplied = StickerService.applyStickerToObject(sticker,
	// findById.get());
	// assertTrue(stickerApplied);
	// // deliberately try twice to see that the sum still is 1
	// stickerApplied = StickerService.applyStickerToObject(sticker,
	// findById.get());
	// assertTrue(stickerApplied);
	//
