...
public class DBUpdate {
	...
	static final String[] versions = { "1.3.0", "1.3.1", "1.3.2", "1.3.3",
			...
			"1.6.3", "1.6.4", "1.7.0", "1.7.1", "1.7.2", "1.8.0", "1.8.1",
			"1.8.2", "1.8.3", "1.8.4", "1.8.5", "1.8.6", "1.8.7", "1.8.8",
	...
	static final String[] cmds = {
			...
			// 1.8.0
			"DROP TABLE PATIENT_GARANT_JOINT;"
					+ "DROP TABLE PLZ;"
					...
					+ "ALTER TABLE HEAP DROP lastupdate;"
					+ "ALTER TABLE HEAP ADD lastupdate BIGINT;"
					+ "ALTER TABLE HEAP2 DROP lastupdate;"
					+ "ALTER TABLE HEAP2 ADD lastupdate BIGINT;"
					...
					+ "ARTICLE_ID      VARCHAR(25)," + "notes           TEXT,"
					+ "image           BLOB" + ");",
			...
			"ALTER TABLE ARTIKEL ADD ValidFrom CHAR(8);"
					...
					+ "deleted        CHAR(1) default '0',"
					+ "ObjectID		VARCHAR(25)," + "ObjectType		VARCHAR(80),"
					+ "Datum			CHAR(8)," + "Outputter		VARCHAR(80),"
					+ "ExtInfo		BLOB);"
	...
	/**
	 * Diese Methode erledigt Datenbankänderungen, die im Rahmen eines Updates
	 * nötig sind Versions enthält eine Versionsliste, cmds ein Kommando für
	 * jede dieser Versionen. Ein Kommando ist entweder
	 * <ul>
	...
	public static void doUpdate() {
		...
		if (dbv == null) {
			...
			ElexisStatus es = new ElexisStatus(
		...
		SqlRunner runner = new SqlRunner(sqlStrings.toArray(new String[0]),
				CoreHub.PLUGIN_ID);
		...
		if (runner.runSql()) {
			...
			// create log message
			log.log("DBUpdate from Version " + dbv + " to Version "
					+ versions[versions.length - 1] + " successful.", Log.INFOS);
		} else {
			log.log("DBUpdate from Version " + dbv + " to Version "
					+ versions[versions.length - 1] + " failed.", Log.ERRORS);
		}
	}
}
