...
import ch.elexis.core.data.Xid.XIDException;
...
public abstract class PersistentObject implements IPersistentObject {
	...
	protected static Logger log = LoggerFactory
			.getLogger(PersistentObject.class.getName());
	private String id;
	...
	static {
		...
		default_lifetime = CoreHub.localCfg.get(Preferences.ABL_CACHELIFETIME,
				CACHE_DEFAULT_LIFETIME);
		if (default_lifetime < CACHE_MIN_LIFETIME) {
			default_lifetime = CACHE_MIN_LIFETIME;
			CoreHub.localCfg.set(Preferences.ABL_CACHELIFETIME,
					CACHE_MIN_LIFETIME);
		}
	 ...
	 * After successful connection, the global Settings (CoreHub.globalCfg) are
	...
	public static boolean connect(final Settings cfg) {
		...
		File demo = new File(base.getParentFile().getParent() + File.separator
				+ "demoDB");
		log.info("Verzeichnis Demo-Datenbank via Hub.getBasePath(): "
				+ demo.getAbsolutePath());
		log.info("osgi.install.area: "
				+ System.getProperty("osgi.install.area"));
		String demo2path = org.eclipse.core.runtime.Platform
		...
		if (!demo.exists()) {
			...
			log.info("Verzeichnis Demo-Datenbank via osgi.install.area: "
					+ demo.getAbsolutePath());
		}
		if (demo.exists() && demo.isDirectory()) {
			j = JdbcLink.createH2Link(demo.getAbsolutePath() + File.separator
					+ "db");
			try {
		...
		try {
			log.info("Current work directory is "
	...
	public static boolean connect(final JdbcLink jd) {
		...
		if (tableExists("CONFIG")) {
			...
			Stm stm = null;
			try {
				String createscript = CoreHub.getBasePath() + File.separator
				...
				if (stm.execScript(is, true, true) == true) {
					CoreHub.globalCfg = new SqlSettings(getConnection(),
							"CONFIG");
					CoreHub.globalCfg.undo();
					CoreHub.globalCfg.set("created",
					...
					if (runningAsTest) {
						...
						String clientEmail = System
								.getProperty("ch.elexis.clientEmail");
						if (clientEmail == null)
						...
						m.set(new String[] { Person.NAME, Person.FIRSTNAME,
								...
								"Dr. med.", Person.MALE, clientEmail,
					...
					if (runningAsTest) {
						...
						JdbcLink jReconnect = new JdbcLink(
								testJdbcLink.getDriverName(),
								testJdbcLink.getConnectString(),
								testJdbcLink.DBFlavor);
					...
					log.error("Kein create script für Datenbanktyp "
		...
		CoreHub.acl.load();
		VersionInfo vi = new VersionInfo(CoreHub.globalCfg.get("dbversion",
				"0.0.0"));
		...
		if (vi.isNewerMinor(v2)) {
			String msg = String
					.format("Die Datenbank %1s ist für eine neuere Elexisversion '%2s' als die aufgestartete '%3s'. Bitte machen Sie ein Update.",
							jd.getConnectString(), vi.version().toString(), v2
									.version().toString());
			log.error(msg);
	...
	/**
	 * Return the Object containing the cdecodeonnection. This should only in
	 * very specific conditions be neccessary, if one needs a direkt access to
	 * the database. It is strongly recommended to use this only very carefully,
	 * as callers must ensure for themselves that their code works with
	...
	public boolean set(final String field, String value) {
		...
		try {
			...
			ElexisStatus status = new ElexisStatus(ElexisStatus.ERROR,
					CoreHub.PLUGIN_ID, ElexisStatus.CODE_NONE, "Fehler bei: "
							+ cmd + "(" + field + "=" + value + ")", ex,
					ElexisStatus.LOG_ERRORS);
	...
	private void setBinaryRaw(final String field, final byte[] value) {
		...
		try {
			...
			throw new PersistenceException(new ElexisStatus(Status.ERROR,
					CoreHub.PLUGIN_ID, ElexisStatus.CODE_NONE,
					"setBytes: Es trat ein Fehler beim Schreiben auf. "
			...
			try {
				...
				ExHandler.handle(e);
				throw new PersistenceException("Could not close statement "
						+ e.getMessage());
			}
		}
	}
	...
	public boolean set(final String[] fields, final String... values) {
		...
		try {
			...
			ElexisStatus status = new ElexisStatus(ElexisStatus.ERROR,
					CoreHub.PLUGIN_ID, ElexisStatus.CODE_NONE, sb.toString(),
	 ...
	 */
	public static void executeSQLScript(String filepath, String plugin)
			throws IOException {
	...
	protected String[] getExportFields() {
		try {
			ResultSet res = getConnection().getStatement().query(
	...
	private ResultSet executeSqlQuery(String sql) {
		...
		try {
			...
			if (je instanceof JdbcLinkResourceException) {
				status.setCode(ElexisStatus.CODE_RESTART
	...
	private static ElexisStatus translateJdbcException(JdbcLinkException jdbc) {
		if (jdbc instanceof JdbcLinkSyntaxException) {
			return new ElexisStatus(ElexisStatus.ERROR, CoreHub.PLUGIN_ID,
					ElexisStatus.CODE_NONE, "Fehler in der Datenbanksyntax.",
					jdbc, ElexisStatus.LOG_ERRORS);
		} else if (jdbc instanceof JdbcLinkConcurrencyException) {
			return new ElexisStatus(ElexisStatus.ERROR, CoreHub.PLUGIN_ID,
					ElexisStatus.CODE_NONE,
					"Fehler bei einer Datenbanktransaktion.", jdbc,
					ElexisStatus.LOG_ERRORS);
		} else if (jdbc instanceof JdbcLinkResourceException) {
			return new ElexisStatus(ElexisStatus.ERROR, CoreHub.PLUGIN_ID,
					ElexisStatus.CODE_NONE,
					"Fehler bei der Datenbankkommunikation.", jdbc,
					ElexisStatus.LOG_ERRORS);
		} else {
			return new ElexisStatus(ElexisStatus.ERROR, CoreHub.PLUGIN_ID,
					ElexisStatus.CODE_NONE,
					"Fehler in der Datenbankschnittstelle.", jdbc,
					ElexisStatus.LOG_ERRORS);
		}
	}
	...
	public static boolean tableExists(String tableName) {
		...
		try {
			...
			log.error(
					"Fehler beim abrufen der Datenbank Tabellen Information.",
					je);
		}
		if (nrFounds > 1) {
			...
			log.error("Tabelle " + tableName + " " + nrFounds
					+ "-mal gefunden!!");
		}
		return nrFounds == 1;
	}
