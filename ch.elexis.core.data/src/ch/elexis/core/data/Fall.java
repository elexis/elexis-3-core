...
public class Fall extends PersistentObject {
	...
	public void setBillingDate(TimeTool dat) {
		set(FLD_RN_PLANUNG,
				dat == null ? null : dat.toString(TimeTool.DATE_GER));
	}
	...
	Fall(final String PatientID, final String Bezeichnung, final String Grund,
		...
		create(null);
		set(new String[] { PATIENT_ID, FLD_BEZEICHNUNG, FLD_GRUND,
				FLD_DATUM_VON }, PatientID, Bezeichnung, Grund,
	 ...
	 *         K,T,D for Kontakt, Text, Date TM Text Multiline TS Text Styled CS
	 *         Combo saved as string CN Combo saved as numeric (selected index)
	 *         LS List items, saved as strings, tab-delimited LN List items,
	 *         saved as numerics, tab-delimited (selected indexes) X CheckBox
	 *         always saved as numeric RS Radios, saved as string RN Radios,
	 *         saved as numeric, selected index
	 */
	...
	/**
	 * Retrieve unused/saved definitions of previously used required and
	 * optional field of this Cases billing system
	 * 
	 * @return a ; separated String of fields name:type where type is one of
	 *         K,T,D for Kontakt, Text, Date TM Text Multiline TS Text Styled CS
	 *         Combo saved as string CN Combo saved as numeric (selected index)
	...
	public String getInfoString(final String name) {
		...
		log.warn("Invalid object in Fall.getInfoString(" + name
				+ "), not castable to String: " + extinfo.get(name));
		return "";
	}
			...
			for (IConfigurationElement ic : list) {
				if (ic.getAttribute("name").startsWith("Tarmed")) { //$NON-NLS-1$ //$NON-NLS-2$
					CoreHub.globalCfg.set(
							Preferences.LEISTUNGSCODES_CFG_KEY
									+ "/KVG/name", //$NON-NLS-1$
							KVG_NAME);
					CoreHub.globalCfg.set(
							Preferences.LEISTUNGSCODES_CFG_KEY
									+ "/KVG/gesetz", //$NON-NLS-1$
							"KVG"); //$NON-NLS-1$
					CoreHub.globalCfg
							.set(Preferences.LEISTUNGSCODES_CFG_KEY
					...
					CoreHub.globalCfg.set(
							Preferences.LEISTUNGSCODES_CFG_KEY
									+ "/UVG/gesetz", //$NON-NLS-1$
							"UVG"); //$NON-NLS-1$

					CoreHub.globalCfg.set(
							Preferences.LEISTUNGSCODES_CFG_KEY
									+ "/IV/name", IV_NAME); //$NON-NLS-1$
					CoreHub.globalCfg
							.set(Preferences.LEISTUNGSCODES_CFG_KEY
					...
					CoreHub.globalCfg.set(
							Preferences.LEISTUNGSCODES_CFG_KEY
									+ "/privat/gesetz", "VVG"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public static void createAbrechnungssystem(final String systemname,
		...
		String key = Preferences.LEISTUNGSCODES_CFG_KEY
	...
	public static String[] getBillingSystemConstants(final String billingSystem) {
		String bc = CoreHub.globalCfg.get(
				Preferences.LEISTUNGSCODES_CFG_KEY + "/" //$NON-NLS-1$
	...
	/**
	 * Retrieve unused/saved definitions of previously used required and
	 * optional field for a given billingSystem
	 * 
	 * @param billingSystem
	 * @return a ; separated String of fields name:type where type is one of
	 *         K,T,D for Kontakt, Text, Date TM Text Multiline TS Text Styled CS
