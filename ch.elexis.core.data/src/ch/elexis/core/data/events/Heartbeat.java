...
public class Heartbeat {
	...
	private Heartbeat() {
		...
		int interval = CoreHub.localCfg.get(Preferences.ABL_HEARTRATE,
				30); //$NON-NLS-1$
		isSuspended = true;
	 ...
	 * 
	 * @param immediately
	 *            true: Sofort einen ersten beat losschicken, false: im normalen
	 *            Rhythmus bleiben.
	 */
	...
	/**
	 * Add listener using the specified frequency. Must be de-regsitered again
	 * using removeListener
	 * 
	 * @param listener
	 * @param frequency
	 *            the frequency to call this listener. One of FREQUENCY_HIGH,
	...
	public interface HeartListener {
		/**
		 * Die Methode heartbeat wird in "einigermassen" regelmässigen (aber
		 * nicht garantiert immer genau identischen) Abständen aufgerufen
		 * 
		 */
