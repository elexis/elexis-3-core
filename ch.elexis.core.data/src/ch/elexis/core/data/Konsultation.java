...
public class Konsultation extends PersistentObject implements
	...
	private boolean isEditable(boolean checkMandant, boolean checkBill,
		...
		if (showError) {
			...
			MessageEvent.fireError("Konsultation kann nicht geändert werden",
					msg);
		}
		...
		try {
			while (rs1.next() == true) {
				...
				if (rs2.next()) {
					...
					try {
						PersistentObject dg = CoreHub.poFactory
	...
	public Result<IVerrechenbar> addLeistung(IVerrechenbar l) {
		if (isEditable(false)) {
			...
			if (result.isOK()) {
				...
				CoreHub.actUser.statForString("LeistungenMFU",
						l.getCodeSystemName());
	...
	public static IDiagnose getDefaultDiagnose() {
		IDiagnose ret = null;
		String diagnoseId = CoreHub.userCfg
				.get(Preferences.USR_DEFDIAGNOSE, "");
