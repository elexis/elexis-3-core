...
/**
 * Contains methods to access and manipulate logback framework dynamically at
 * run-time. Here 'dynamically' means without referencing the logback JAR, but
 * using it if found in the classpath.
 * 
...
public final class LogbackUtils {
	...
	/**
	 * Dynamically sets the logback log level for the given class to the
	 * specified level.
	 * 
	 * @param loggerName
	 *            Name of the logger to set its log level. If blank, root logger
	 *            will be used.
	 * @param logLevel
	 *            One of the supported log levels: TRACE, DEBUG, INFO, WARN,
	 *            ERROR, FATAL, OFF. {@code null} value is considered as 'OFF'.
	 */
	public static boolean setLogLevel(String loggerName, String logLevel) {
		String logLevelUpper = (logLevel == null) ? "OFF" : logLevel
				.toUpperCase();
		...
		try {
			...
			if ((loggerName == null) || loggerName.trim().isEmpty()) {
				loggerName = (String) getFieldVaulue(LOGBACK_CLASSIC_LOGGER,
						"ROOT_LOGGER_NAME");
			}
			...
			Object logLevelObj = getFieldVaulue(LOGBACK_CLASSIC_LEVEL,
					logLevelUpper);
			if (logLevelObj == null) {
			...
			logger.debug("Log level set to {} for the logger '{}'",
					logLevelUpper, loggerName);
			return true;
		} catch (Exception e) {
			logger.warn("Couldn't set log level to {} for the logger '{}'",
					logLevelUpper, loggerName, e);
			return false;
		}
	}
