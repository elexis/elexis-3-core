...
public class Hub extends AbstractUIPlugin {
	...
	public static void postShutdown() {
		...
		if ((shutdownJobs != null) && (shutdownJobs.size() > 0)) {
			...
			MessageDialog dlg = new MessageDialog(shell,
					Messages.Hub_title_configuration, Dialog.getDefaultImage(),
					Messages.Hub_message_configuration, SWT.ICON_INFORMATION,
					new String[] {}, 0);
	...
	/*
	 * Sets the window title to a nicely formatted string containt the family
	 * name, name, age and its code
	 */
	...
	/**
	 * get the currently active Shell. If no such Shell exists, it will be
	 * created using dhe default Display.
	 * 
	...
	/**
	 * Add a ShutdownJob to the list of jobs that has to be done after the
	 * Elexis workbench was shut down.
	 * 
	 * @param job
	 */
