...
public class Stickers extends PreferencePage implements IWorkbenchPreferencePage {
	...
	void setSticker(Sticker et){
		...
		if (et == null) {
			...
			cFore.setBackground( ((UiSticker)et).getForeground());
			cBack.setBackground( ((UiSticker)et).getBackground());
	...
	protected Control createContents(Composite parent){
		...
		cImage.addPaintListener(new PaintListener() {
			...
			public void paintControl(PaintEvent e){
				GC gc = e.gc;
				if (act != null) {
					Image img = ((UiSticker)act).getImage();
					if (img != null) {
			...
			public void widgetSelected(SelectionEvent e){
				if (act != null) {
					...
					if (imc.open() == Dialog.OK) {
						 ((UiSticker)act).setImage(imc.getSelection());
						setSticker(act);
					}
				}
			}
		...
		bFore.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e){
				if (act != null) {
					...
					RGB rgb = cd.open();
					if (rgb != null) {
						 ((UiSticker)act).setForeground(UiDesk.createColor(rgb));
					}
					setSticker(act);
				}
			}
		...
		bBack.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e){
				if (act != null) {
					...
					RGB rgb = cd.open();
					if (rgb != null) {
						 ((UiSticker)act).setBackground(UiDesk.createColor(rgb));
					}
					setSticker(act);
				}
			}
