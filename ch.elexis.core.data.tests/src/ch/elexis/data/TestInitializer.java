...
public class TestInitializer {
	...
	public static JdbcLink initTestDBConnection(String dbflavor){
		...
		if (dbflavor == FLAVOR_H2_MEM) {
			...
			link = JdbcLink.createMySqlLink("localhost", "elexis_ut");
		} else if (dbflavor == FLAVOR_POSTGRES) {
			link = JdbcLink.createPostgreSQLLink("localhost", "elexis_ut");
		}
		...
		try {
			boolean connectionOk = link.connect("elexis_ut", "elexisTest");
			if (connectionOk) {
				return link;
			}
		} catch (Exception jle) {
