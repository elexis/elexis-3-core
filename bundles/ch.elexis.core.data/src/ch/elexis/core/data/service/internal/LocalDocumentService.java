...
public class LocalDocumentService implements ILocalDocumentService {
	...
	private boolean tryBackup(Path path) {
		try {
			...
			FileUtils.copyFile(src, new File(backupDir, "bak_" + System.currentTimeMillis() + "_" + src.getName()));
			return true;
		} catch (IOException e) {
	...
	public Optional<InputStream> getContent(Object documentSource) {
		...
		if (file != null) {
			try {
				return Optional.of(new ByteArrayInputStream(Files.readAllBytes(Paths.get(file.getAbsolutePath()))));
			} catch (IOException e) {
	 ...
	 * @return
	 */
	private Optional<File> writeLocalFile(String fileName, InputStream content, IConflictHandler conflictHandler,
			boolean readOnly) {
	...
	private String getFileName(Object documentSource) {
		...
		try {
			...
			if (mimeMethod != null) {
				sb.append("." + getFileEnding((String) mimeMethod.invoke(documentSource, new Object[0])));
			} else {
	...
	private boolean isGetNameMethod(Method method) {
		...
		String lowerName = method.getName().toLowerCase();
		if (lowerName.contains("betreff") || lowerName.contains("titel") || lowerName.contains("title")) {
			return true;
		}
		return false;
	}
