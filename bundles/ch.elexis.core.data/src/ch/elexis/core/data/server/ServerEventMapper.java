...
/**
 * Maps internal {@link ElexisEvent} instances to outgoing ElexisEvent instances
 * sent to Elexis-Server
 */
public class ServerEventMapper {
	...
	public static @Nullable ch.elexis.core.common.ElexisEvent mapEvent(ElexisEvent ee) {
		...
		if (object != null) {
			...
			remoteEvent.getProperties().put(ElexisEventTopics.PROPKEY_CLASS, object.getClass().getName());
		}

		ContextServiceHolder.get().getActiveUser()
				.ifPresent(u -> remoteEvent.getProperties().put(ElexisEventTopics.PROPKEY_USER, u.getId()));
		return remoteEvent;
	}
