 ...
 * however, not considering single object deviating cache times as propagated by
...
public class GuavaCache<K> implements IPersistentObjectCache<K> {
	...
	public GuavaCache(long duration, TimeUnit unit) {
		shortTermCache = CacheBuilder.newBuilder().softValues().recordStats().expireAfterWrite(duration, unit).build();
	}
