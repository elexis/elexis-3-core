...
public class MultiGuavaCache<K> implements IPersistentObjectCache<K> {
	...
	public MultiGuavaCache(long duration, TimeUnit unit) {
		if (STAT_ENABLED) {
			shortTermCache = CacheBuilder.newBuilder().softValues().recordStats().expireAfterWrite(duration, unit)
					.build();
			longTermCache = CacheBuilder.newBuilder().softValues().recordStats().maximumSize(Long.MAX_VALUE).build();
		} else {
			shortTermCache = CacheBuilder.newBuilder().softValues().expireAfterWrite(duration, unit).build();
			longTermCache = CacheBuilder.newBuilder().softValues().maximumSize(Long.MAX_VALUE).build();
		}
	}
