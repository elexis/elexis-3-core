...
public class ZusatzAdresse extends PersistentObject implements ITransferable<ZusatzAdresseDTO> {
	...
	private static final String[] FIELDS = new String[]{KONTAKT_ID, STREET1, STREET2, COUNTRY, PLACE, ZIP, TYPE,
			POSTAL_ADDRESS};

	static {
		addMapping(TABLENAME, KONTAKT_ID, STREET1, STREET2, ZIP, PLACE, COUNTRY, TYPE, POSTAL_ADDRESS);
	}
	...
	public String getEtikette(boolean withName, boolean multiline) {
		return getEtikette(withName, multiline, get(KONTAKT_ID), get(STREET1), get(COUNTRY), get(ZIP), get(PLACE));
	}

	public String getEtikette(boolean withName, boolean multiline, String kontaktId, String street1, String country,
			String zip, String place) {
	...
	public String getLabel() {
		...
		if (get(TYPE) != null) {
			return LocalizeUtil.getLocaleText(AddressType.get(NumberUtils.toInt(get(TYPE)))) + ": " + etikette;
		}
		return etikette;
	}
	...
	public void persistDTO(ZusatzAdresseDTO zusatzAdresseDTO) throws ElexisException {
		...
		if (zusatzAdresseDTO.getKontaktId() == null) {
			throw new ElexisException(ZusatzAdresse.class, "Kontakt not definied", ElexisException.EE_NOT_SUPPORTED,
					true);
		}
		...
		set(FIELDS, zusatzAdresseDTO.getKontaktId(), zusatzAdresseDTO.getStreet1(), zusatzAdresseDTO.getStreet2(),
				zusatzAdresseDTO.getCountry(), zusatzAdresseDTO.getPlace(), zusatzAdresseDTO.getZip(),
				String.valueOf(zusatzAdresseDTO.getAddressType().getValue()), zusatzAdresseDTO.getPostalAddress());
	}
}
