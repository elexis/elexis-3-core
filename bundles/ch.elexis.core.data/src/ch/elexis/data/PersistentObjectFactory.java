...
public class PersistentObjectFactory implements IExecutableExtension {
	...
	/**
	 * Create a template of the provided class type, without creating a
	 * corresponding database entry. If the core is not able to create the resp.
	 * class, all plug-ins contributing a PersistentObjectFactory will be queried.
	 *
	...
	public PersistentObject createTemplate(Class type) {
		...
		try {
			...
			List<PersistentObjectFactory> contributedFactories = Extensions
			...
			ElexisStatus status = new ElexisStatus(ElexisStatus.ERROR, CoreHub.PLUGIN_ID, ElexisStatus.CODE_NONE,
					"create: Couldn't create object " + ex.getMessage(), ex, ElexisStatus.LOG_ERRORS);
	...
	/**
	 * Helper-Funktion, die Objekte eines beliebigen abgeleiteten Typs mit
	 * beliebigen Feldvorgaben erstellen kann.
	 *
	 * @param typ    Die Klasse des zu erstellenden Objekts
	 * @param fields Die initial zu belegenden Felder. ID darf nicht angegeben
	 *               werden.
	...
	/**
	 * Create an object of a derived class given a pseudo de-serialization code,
	 * e.g.
	 * <code>ch.elexis.artikel_ch.data.Medikament::ca8bb5c27bdd67d5f011821</code>.
	 * If the object can not be created by the core, all plug-ins contributing a
	 * {@link #PersistentObjectFactory()} are queried.
	 *
	 * @param code the storeToString as shown in the above example
	 ...
	 *
	 * @param code         the storeToString as shown in the above example
	 * @param dbConnection the db connection used by the created PersistenObject, if
	...
	public PersistentObject createFromString(String code, DBConnection dbConnection) {
		...
		try {
			...
			ret = (PersistentObject) (load.invoke(null, new Object[] { ci[1] }));
			ret.setDBConnection(dbConnection);
			return ret;
	...
	/**
	 * Ask the plug-in local classloader to return an instance of a Class as given
	 * by the first part of the storeToString (e.g.
	 * ch.elexis.eigenartikel.Eigenartikel::392393253959)
	 *
