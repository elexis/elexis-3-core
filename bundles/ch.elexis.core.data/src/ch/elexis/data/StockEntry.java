...
public class StockEntry extends PersistentObject implements IStockEntry {
	...
	static {
		addMapping(TABLENAME, FLD_STOCK, FLD_ARTICLE_TYPE, FLD_ARTICLE_ID, FLD_MIN + "=S:N:" + FLD_MIN,
				FLD_CURRENT + "=S:N:" + FLD_CURRENT, FLD_MAX + "=S:N:" + FLD_MAX,
				FLD_FRACTIONUNITS + "=S:N:" + FLD_FRACTIONUNITS, FLD_PROVIDER);
	}
	...
	public StockEntry(Stock stock, PersistentObject article) {
		String provider = ConfigServiceHolder.getGlobal(Preferences.INVENTORY_DEFAULT_ARTICLE_PROVIDER, null);
		String[] fields = new String[] { FLD_STOCK, FLD_ARTICLE_TYPE, FLD_ARTICLE_ID, FLD_PROVIDER };
		String[] values = new String[] { stock.getId(), article.getClass().getName(), article.getId(),
	...
	public Artikel getArticle() {
		...
		if (StringUtils.isNotBlank(vals[1])) {
			return (Artikel) CoreHub.poFactory.createFromString(vals[0] + StringConstants.DOUBLECOLON + vals[1]);
		}
		return null;
	}
	...
	public void setArticle(Artikel article) {
		if (article != null) {
			set(new String[] { FLD_ARTICLE_TYPE, FLD_ARTICLE_ID }, article.getClass().getName(), article.getId());
		} else {
			set(new String[] { FLD_ARTICLE_TYPE, FLD_ARTICLE_ID }, null, null);
		}
	}
	...
	public void setCurrentStock(int currentStock) {
		// ACHTUNG
		// ConfigServiceHolder.getGlobal(Preferences.INVENTORY_CHECK_ILLEGAL_VALUES
		/**
		 * Istbestand setzen. Wenn INVENTORY_CHECK_ILLEGAL-VALUES gesetzt ist, erscheint
		 * eine Warnung, wenn der Istbestand unter null komt.
		 *
		 * @param s Wieviele Packungen tatsächlich auf Lager sind
		 */
