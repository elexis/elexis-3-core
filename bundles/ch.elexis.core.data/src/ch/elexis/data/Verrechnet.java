 ...
 * geändert werden (Rabatt etc.) Nebst VK_Scale, welche in der Schweiz dem
 * taxpunktwert entspricht, können noch externe und interne zusätzlich
 * Skalierungen angewendet werden. PrimaryScalefactor wird beispielsweise für
 * %-Reduktionen oder Zusschläge gemäss Tarmed verwendet, SecondaryScalefactor
 * kann ein Rabatt oder ein Privatzuschschlag sein.
 *
 * @author gerry
 *
 */
public class Verrechnet extends PersistentObject {
	...
	public Verrechnet(final IVerrechenbar iv, final Konsultation kons, final int zahl) {
		...
		String[] fields = new String[] { KONSULTATION, LEISTG_TXT, LEISTG_CODE, CLASS, COUNT, COST_BUYING,
				SCALE_TP_SELLING, SCALE_SELLING, PRICE_SELLING, SCALE, SCALE2, USERID };
		String[] values = new String[] { kons.getId(), iv.getText(), iv.getId(), iv.getClass().getName(),
				Integer.toString(zahl), iv.getKosten(dat).getCentsAsString(), Integer.toString(tp),
				Double.toString(factor), Long.toString(preis), "100", "100", CoreHub.getLoggedInContact().getId() };
	...
	/**
	 * set the primary scale factor (usually system specific or "internal" to the
	 * code system NOTE: This ist NOT identical to the multiplier or "Taxpunkt". The
	 * final price will be calculated as VK_PREIS * VK_SCALE * primaryScale *
	 * secondaryScale
	 *
	 * @param scale the new scale value as x.x
	 */
	...
	public void setStandardPreis() {
		...
		long preis = Math.round(tp * factor);
		set(new String[] { SCALE_SELLING, SCALE_TP_SELLING, PRICE_SELLING }, Double.toString(factor),
				Integer.toString(tp), Long.toString(preis));
	}
	...
	/**
	 * Change the count for this service or article. If it ist an Artikel, the store
	...
	/**
	 * Change the count for this service or article, considering the rules given by
	 * the resp. optifiers.
	 *
	 * @param neuAnzahl
	 * @return
	...
	public boolean isInstance(final IVerrechenbar tmpl) {
		...
		get(new String[] { CLASS, LEISTG_CODE }, res);
