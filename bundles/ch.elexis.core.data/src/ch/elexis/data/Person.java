...
public class Person extends Kontakt {
	...
	static {
		addMapping(Kontakt.TABLENAME, NAME + "=" + Kontakt.FLD_NAME1, FIRSTNAME + "=" + Kontakt.FLD_NAME2,
				"Zusatz 		=" + Kontakt.FLD_NAME3, BIRTHDATE + "=	S:D:Geburtsdatum", SEX, MOBILE + "=NatelNr", //$NON-NLS-2$ //$NON-NLS-3$
	 ...
	 *
	 * @param name    will be checked for non-alphabetic characters and may not be
	 *                empty
	...
	public Person(String name, String vorname, TimeTool gebDat, String s) throws PersonDataException {
		...
		if ((!StringTool.isNothing(vorname)) && (!vorname.matches("[" + StringTool.wordChars + "\\s-]+"))) { //$NON-NLS-1$ //$NON-NLS-2$
	...
	public String getInitials(int num) {
		...
		ret.append((vorname.length() > num - 1) ? vorname.substring(0, num) : vorname).append(".(").append(sex)
				.append("), ").append(geb);
		return ret.toString();
	}
	 ...
	 *
	 * Ein allfälliger Titel wie Dr. med. kommt nach Name und Vorname, damit die
	 * Suche bei der Patientsicht nach Namen und Person funktioniert
	 */
	public String getPersonalia() {
		...
		if (StringTool.isNothing(vals[3])) {
			...
			String localiced = shortVal.contentEquals(Patient.FEMALE) ? Messages.Patient_female_short
					: Messages.Patient_male_short;
	...
	public static class PersonDataException extends Exception {
		...
		static final String[] causes = new String[] { NAME, FIRSTNAME, BIRTHDATE, "Geschlecht (m oder w)" }; //$NON-NLS-1$
