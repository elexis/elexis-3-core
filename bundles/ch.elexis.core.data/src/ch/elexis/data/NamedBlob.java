...
public class NamedBlob extends PersistentObject {
	...
	/**
	 * return the contents as String (will probably fail if the data was not stored
	 * using putString)
	 *
	...
	/**
	 * Load or create a NamedBlob with a given Name. Caution: This will never return
	 * an inexistent NamedBlob, because it will be created if necessary. Use
	 * exists() to check, whether a NamedBlob exists.
	 *
	 ...
	 * @return list with all NamedBlobs wohse name matches (case sensitively) the
	 *         prefix
	 */
	...
	/**
	 * remove all BLOBS with a given name prefix and a last write time older than
	 * the given value needs the administrative right AC_PURGE
	 *
	 * @param prefix
	 * @param older
	 */
