...
public class Patient extends Person {
	...
	public static final String FLD_Mobile = "Mobil-Tel.";
	...
	static {
		addMapping(Kontakt.TABLENAME, FLD_DIAGNOSES + "    	=S:C:Diagnosen", FLD_PERS_ANAMNESE + "	=S:C:PersAnamnese",
				...
				FLD_BALANCE + "			=LIST:PatientID:KONTO", FLD_GROUP, FLD_PATID, Kontakt.FLD_IS_PATIENT);
	}
	...
	/**
	 * Dieser oder der folgende Konstruktor sollte normalerweise verwendet werden,
	 * um einen neuen, bisher noch nicht in der Datenbank vorhandenen Patienten
	 * anzulegen.
	 *
	 * @param Name
	 * @param Vorname
	 ...
	 * @param s       sexe must be entered as (in German) m for male and w for
	 *                female (weiblich)
	...
	public Konsultation getLastKonsultation() {
		...
		if (getId() != null) {
			PreparedStatement preparedStatement = PersistentObject.getDefaultConnection().getPreparedStatement(
	 ...
	 * vergebene Nummer sein, oder ein aus den Personalien gebildetes Kürzel. Dieser
	 * Code kann beispielsweise als Index für die Archivierung der KG's in
	 * Papierform verwendet werden.
	 *
	...
	public String getPatCode() {
		...
		while (true) {
			...
			String pid = getDBConnection().queryString("SELECT WERT FROM CONFIG WHERE PARAM='PatientNummer'");
	...
	/**
	 * to be used by the platzhalter system, allows for presentation of the current
	 * balance of this patient by using [Patient.Konto]
	 *
	...
	/**
	 * Calculates a possibly available account excess. (This value may be added to a
	 * bill as prepayment.)
	 * <p>
	 * Considers all overpaid bills and account transactions not bound to a bill.
	 * The garant of the bill must be the patient itself. (Bills not yet paid or
	 * partly paid are not considered.)
	 * <p>
	 * This value is not the same as the current account balance, since we ignore
	 * outstanding debts of not yet paid bills.
	 *
	...
	public static Patient loadByPatientID(String patientNr) {
		String patID = new Query<Patient>(Patient.class).findSingle(Patient.FLD_PATID, Query.EQUALS, patientNr);
	...
	protected String getConstraint() {
		return new StringBuilder(Kontakt.FLD_IS_PATIENT).append(Query.EQUALS).append(JdbcLink.wrap(StringConstants.ONE))
				.toString();
	}

	@Override
	protected void setConstraint() {
		set(new String[] { Kontakt.FLD_IS_PATIENT, Kontakt.FLD_IS_PERSON }, StringConstants.ONE, StringConstants.ONE);
	}
	 ...
	 *
	 * This implementation returns "<Vorname> <Name>" for the sort label, and calls
	 * getPersonalia() for the long label.
	 *
	 ...
	 * verknüpften Daten gelöscht (Labor, Rezepte, AUF, Rechnungen etc.) Plugins,
	 * welche patientenspezifische Daten speichern, sollten diese ebenfalls löschen
	 * (sie erhalten einen ObjectEvent)
	 *
	 * @param force bei true wird der Patient auf jeden Faöll gelöscht, bei false
	...
	private boolean delete_dependent() {
		for (LabResult lr : new Query<LabResult>(LabResult.class, LabResult.PATIENT_ID, getId()).execute()) {
			lr.delete();
		}
	...
	/**
	 * Eine Auftragsnummer erstellen. Diese enthält die Patientennummer ergänzt mit
	 * der Modulo10-Prüfsumme über diese Nummer, plus die aktuelle Uhrzeit als -hhmm
	 *
	...
	public Kontakt getStammarzt() {
		...
		return (getExtInfoStoredObjectByKey(FLD_EXTINFO_STAMMARZT) != null)
				? Kontakt.load((String) getExtInfoStoredObjectByKey(FLD_EXTINFO_STAMMARZT))
				: null;
	}
