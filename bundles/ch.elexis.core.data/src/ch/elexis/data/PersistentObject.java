...
public abstract class PersistentObject implements IPersistentObject {
	...
	/**
	 * predefined property that holds an automatically updated field containing the
	 * last update of this object as long value (milliseconds as in Date())
	 */
	 ...
	 *
	 * In the first place, the method checks if there is a demoDB in the Elexis base
	 * directory. If found, only this database will be used. If not, connection
	 * parameters are taken from the provided Settings. If there ist no database
	 * found, it will be created newly, using the createDB-Script. After successful
	 * connection, the defaultconnection is set and the Settings (CoreHub.globalCfg)
	 * are linked to the database.
	 *
	 * For automated testing the following rules apply:
	 *
	 * The methods check whether the properties ch.elexis.* are set. If set, Elexis
	 * will open the corresponding database. E.g -Dch.elexis.username=test
	 * -Dch.elexis.password=test -Dch.elexis.dbUser=elexis
	 * -Dch.elexis.dbPw=elexisTest -Dch.elexis.dbFlavor=mysql
	 ...
	 *
	 * @param dbFlavor   either <code>mysql</code>, <code>postgresql</code> or
	 *                   <code>h2</code>
	 * @param dbSpec     connection string fitting to dbFlavor, e.g.
	 *                   <code>jdbc:postgresql://192.168.0.3:5432/elexis</code>
	 * @param dbUser     the <code>username</code> to connect to the database with
	 * @param dbPw       the <code>password</code> to connect to the database with
	 * @param exitOnFail
	 * @return
	 * @since 3.0.0
	 */
	 ...
	 * @since 3.10
	 */
	public static boolean connect(DataSource dataSource) throws SQLException {
		JdbcLink jdbcLink = new JdbcLink(dataSource);
		DBConnection dbConnection = new DBConnection();

		dbConnection.setJdbcLink(jdbcLink);
		dbConnection.setDBFlavor(dataSource.getConnection().getMetaData().getDatabaseProductName());
		dbConnection.setDBDriver("datasource: " + dataSource.getConnection().getMetaData().getDriverName()); //$NON-NLS-1$
		dbConnection.setDBConnectString(dataSource.getConnection().getMetaData().getURL());
	 ...
	 *
	 * @param connection the already connected DBConnection
	 * @return
	 */
	 ...
	 * Zuordnungen</li>
	 * <li><code>Variable=EXT:tabelle:feld</code> - Das Feld ist in der genannten
	 * externen Tabelle
	 * </ul>
	 */
	 ...
	 * zwei Sekunden beansprucht werden, dann wird sie gelöst. Dies ist eine sehr
	 * teure Methode, die eigentlich nur notwendig ist, weil es keine
	 * standardisierte JDBC-Methode für Locks gibt... Die Sperre ist kooperativ: Sie
	 * verhindert konkurrierende Zugriffe nicht wirklich, sondern verlässt sich
	 * darauf, dass Zugreifende freiwillig zuerst die Sperre abfragen. Sie bezieht
	 * sich auch nicht direkt auf eine bestimmte Tabelle, sondern immer nur auf eine
	 * willkürliche frei wählbare Bezeichnung. Diese muss für jedes zu schützende
	 * Objekt standardisiert werden.
	 *
	...
	public static synchronized String lock(final String name, final boolean wait) {
		...
		try {
			while (true) {
				...
				String oldlock = stm
						.queryString("SELECT wert FROM CONFIG WHERE param=" + getConnection().wrapFlavored(lockname));
				if (!StringTool.isNothing(oldlock)) {
					...
					if (age > 2000L) { // Älter als zwei Sekunden -> Löschen
						stm.exec("DELETE FROM CONFIG WHERE param=" + getConnection().wrapFlavored(lockname));
					} else {
						if (wait == false) {
							return null;
						} else {
	...
	public static synchronized boolean unlock(final String name, final String id) {
		...
		if (res[0].equals(id)) {
			getConnection().exec("DELETE FROM CONFIG WHERE param=" + getConnection().wrapFlavored(lockname));
			return true;
		}
		return false;
	}
	...
	/**
	 * Jede abgeleitete Klasse muss deklarieren, in welcher Tabelle sie gespeichert
	 * werden will.
	 *
	...
	/**
	 * Die eindeutige Identifikation dieses Objektes/Datensatzes liefern. Diese ID
	 * wird jeweils automatisch beim Anlegen eines Objekts dieser oder einer
	...
	/**
	 * Konstruktor mit vorgegebener ID (zum Deserialisieren) Wird nur von xx::load
	 * gebraucht.
	 */
	...
	/**
	 * Objekt in einen String serialisieren. Diese Standardimplementation macht eine
	 * "cheap copy": Es wird eine Textrepräsentation des Objektes erstellt, mit
	 * deren Hilfe das Objekt später wieder aus der Datenbank erstellt werden kann.
	 * Dies funktioniert nur innerhalb derselben Datenbank.
	 *
	 ...
	 * database and therefore is much more costly than the simple instantiation of a
	 * PersistentObject
	 *
	 ...
	 *
	 * @return true wenn es existiert, false wenn es nicht existiert oder gelöscht
	 *         wurde
	 */
	...
	/**
	 * return the "best" xid for a given object. This is the xid with the highest
	 ...
	 * @param updateIfExists if true update values if Xid with same domain and
	 *                       domain_id exists. Otherwise the method will fail if a
	 *                       collision occurs.
	 ...
	 */
	private final String queryStickersString = "SELECT etikette FROM " + Sticker.FLD_LINKTABLE + " WHERE obj=?";
	...
	public void removeSticker(ISticker et) {
		...
		if (ret.contains(et)) {
			DBConnection dbConnection = getDBConnection();
			String remove = "DELETE FROM " + Sticker.FLD_LINKTABLE + " WHERE obj=" + getWrappedId() + " AND etikette="
					+ getDBConnection().getJdbcLink().wrapFlavored(et.getId());
	...
	public void addSticker(ISticker st) {
		...
		if (!ret.contains(st)) {
			DBConnection dbConnection = getDBConnection();
			String update = "INSERT INTO " + Sticker.FLD_LINKTABLE + " (obj,etikette,lastupdate) VALUES ("
					+ getWrappedId() + "," + getDBConnection().getJdbcLink().wrapFlavored(st.getId()) + ","
	...
	/**
	 * Aus einem Feldnamen das dazugehörige Datenbankfeld ermitteln, beinhaltet das
	 * jeweilige Prefix
	 *
	 ...
	 * @param tableName
	 * @param field
	 * @param markAsError whether to return ERROR* or <code>null</code> if no entry
	 *                    found
	 * @return
	 * @since 3.2
	 */
	 ...
	 * gelesen. Dann werden weitere Lesezugriffe während der <i>lifetime</i> aus dem
	 * cache bedient, um die Zahl der Datenbankzugriffe zu minimieren. Nach Ablauf
	 * der lifetime erfolgt wieder ein Zugriff auf die Datenbank, wobei auch der
	 * cache wieder erneuert wird. Wenn das Feld nicht als Tabellenfeld existiert,
	 * wird es in EXTINFO gesucht. Wenn es auch dort nicht gefunden wird, wird eine
	...
	/**
	 * Similar to {@link PersistentObject#get(String)}, but never use cache,
	 * checkNull or decode. Simply read the value from the database. <b>Only use
	 * with non encoded fields!</b>
	 *
	 * @param field
	 * @return
	 */
	...
	/**
	 * Set a value in the {@link #FLD_EXTINFO} field, will create an ExtInfo field
	 * if required
	 *
	 * @param key
	 ...
	 *
	 * @param field the name of the field to be tested
	 ...
	 * @param newVal the new state to save to the cb, one of TristateBoolean
	 *               (TRUE/FALSE/UNDEF)
	 * @author H. Marlovits
	 * @since 3.0.0
	 */
	 ...
	 *
	 * @param field          the field, as provided in the mapping declaration
	 * @param reverse        if <code>true</code> perform reverse ordering
	 * @param includeDeleted if <code>true</code> include objects marked as deleted
	 * @return
	 * @since 3.2
	 */
	public List<String> getList(final String field, final boolean reverse, final boolean includeDeleted) {
		StringBuffer sql = new StringBuffer();
		String mapped = map(field);
		if (mapped.startsWith("LIST:")) {
			...
			if (m.length > 2) {
				...
				if (!includeDeleted) {
					sql.append("deleted=").append(getDBConnection().getJdbcLink().wrapFlavored("0")).append(" AND ");
				}
	 ...
	 * @return eine Liste aus String-Arrays, welche jeweils die ID des gefundenen
	 *         Objekts und den Inhalt der Extra-Felder enthalten. Null bei
	 *         Mapping-Fehler
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Ein Feld in die Datenbank übertragen. Gleichzeitig Cache-update Die Tabelle
	 * wird über getTableName() erfragt.
	 *
	...
	public boolean set(final String field, String value) {
		...
		if (value == null) {
			...
			sql.append(mapped);
			sql.append("=NULL, " + FLD_LASTUPDATE + "=" + Long.toString(ts) + " WHERE ID=").append(getWrappedId());
	...
	/**
	 * Eine Hashtable speichern. Diese wird zunächst in ein byte[] geplättet, und so
	 * gespeichert.
	 *
	 * @param field
	 * @param map
	...
	/**
	 * Eine Element einer n:m Verknüpfung eintragen. Zur Tabellendefinition wird das
	 * mapping verwendet.
	 *
	 ...
	 * @param extra Definition der zusätzlichen Felder der Joint-Tabelle. Jeder
	...
	/**
	 * Set multiple elements on an n:m relation. Use the table definition for
	 * mapping.
	 *
	 * @param field the n:m field of the entries to insert to
	 * @param oID   ids of the targeted objects to which the entry should be added
	 * @param extra
	 * @return 0 in case of error
	 * @since 3.4
	 */
	public int addAllToList(final String field, final List<String> objectIds, final String... extra) {
		String mapped = map(field);
				...
				if (m.length > 3) {
					...
					if (extra != null) {
						for (String s : extra) {
							...
							tail.append(",").append(getDBConnection().getJdbcLink().wrapFlavored(def[1]));
						}
					}
	 ...
	 *
	 * @param customID Wenn eine ID (muss eindeutig sein!) vorgegeben werden soll.
	...
	/**
	 * Create a new object, persisting it into database, including the given fields
	 * and values.
	 *
	 * @param customID  if <code>null</code> generates an ID, else uses the provided
	 * @param fields    the fields to include in the insert statement, does
	 *                  currently partially support special field types as defined
	 *                  in {@link #addMapping(String, String...)}. Use
	 *                  <code>null</code> if not applied
	 * @param values    the values, the length of this array must be equal to the
	 *                  fields array length. Use <code>null</code> if not applied
	 * @param sendEvent send ElexisEvent.EVENT_CREATE
	 * @return <code>true</code> if operation successful
	 * @since 3.2
	 * @since 3.5 supports date (S:D:) special field type, please be sure to use a
	 *        field value in format TimeTool.DATE_COMPACT
	 */
	protected boolean create(final String customID, final String[] fields, final String[] values,
		...
		sql.append(fieldS.stream().map(s -> map(s)).map(fixDate).reduce((u, t) -> u + StringConstants.COMMA + t).get());
		sql.append(") VALUES (");
	...
	/**
	 * Ein Objekt und ggf. dessen XID's aus der Datenbank löschen the object is not
	 * deleted but rather marked as deleted. A purge must be applied to remove the
	 * object really
	 *
	 * @return true on success
	 */
	...
	/**
	 * Effectively removes the object from the database by performing an SQL delete
	 * call.
	 *
	...
	public boolean removeFromDatabase() {
		...
		int result = getDBConnection().exec("DELETE FROM " + getTableName() + " WHERE ID=" + getWrappedId());
		return (result == 1);
	}
	...
	/**
	 * We can undelete any object by simply clearing the deleted-flag and reanimate
	 * dependend XID's
	 *
	 * @return true on success
	 */
	 ...
	 * @param values the storage array for the fields
	 ...
	 *
	 * @param fields          HashMap mit name,wert paaren für die Felder
	 * @param mode            Testmodus (MATCH_EXACT, MATCH_BEGIN, MATCH_REGEXP,
	 *                        MATCH_CONTAIN oder MATCH_AUTO)
	 * @param bSkipInexisting don't return false if a fieldname is not found but
	 *                        skip this field instead
	...
	/**
	 * Get a unique key for a value, suitable for identifying a key in a cache. The
	 * current implementation uses the table name, the id of the PersistentObject
	 * and the field name.
	 *
	 ...
	 *
	 * @param updatedAttribute the attribute that was updated or <code>null</code>
	 * @since 3.1
	 */
	...
	/**
	 * Utility function to create or modify a table consistently. Should be used by
	 * all plugins that contribute data types derived from PersistentObject
	 *
	 * @param sqlScript create string
	 */
	 ...
	 * try { final ByteArrayInputStream bais; bais = new
	 * ByteArrayInputStream(sqlScript .getBytes("UTF-8")); if
	 * (getConnection().execScript(bais, true, false) == false) { SWTHelper
	 * .showError("Datenbank-Fehler", "Konnte Datenbank-Script nicht ausführen");
	 ...
	 * consistentliy To make sure dependent data are deleted as well, we call each
	 * object's delete operator individually before dropping the table
	 *
	 ...
	 * is java-specific, but stable through jre Versions (serialVersionUID:
	 ...
	 * <p>
	 * <b>ATTENTION:</b> JdbcLinkResourceException will trigger a restart of Elexis
	 * in at.medevit.medelexis.ui.statushandler.
	 * </p>
	 *
	 * @param sql
	 * @return
	 ...
	 * Utility procedure
	 *
	 * @param tableName name of the table to check existence for
	 */
	...
	public static boolean tableExists(String tableName, boolean considerViews) {
		...
		try (Connection conn = defaultConnection.getConnection()) {
			...
			String[] searchBase;
			if (considerViews) {
				searchBase = new String[] { "TABLE", "VIEW" };
			} else {
				searchBase = new String[] { "TABLE" };
			}
	 ...
	 *
	 * @param field name, must map to a database column, see
	...
	/**
	 * Clear all attributes that have been cached for this entity. Must be
	 * re-implemented by a subclass to support. See e.g. Reminder
	 */
