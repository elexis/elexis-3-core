...
/**
 * Ein Rezept besteht aus einem Mandanten, einem Patienten, einem Datum und
 * einer Prescription-Liste Aus kompatibilitätsgründen wird in Moment noch der
 * RpText mitgeschleppt
 * 
 * @author Gerry
 * 
 */
public class Rezept extends PersistentObject {
	...
	/**
	 * Den "Brief" liefern. Dieser existiert, wenn das Rezept mindestens einmal
	 * gedruckt wurde und ist die Print-Repräsentation mit etwaigen manuellen
	 * Änderungen
	 * 
	...
	public List<Prescription> getLines() {
		...
		return ret.stream().sorted((p1, p2) -> Integer.compare(p1.getRecipeOrder(), p2.getRecipeOrder()))
	...
	/**
	 * Add a prescription to the recipe. The Prescription will be marked as recipe
	 * entry, and will be stopped immediately. The prescription will be deleted if
	 * it is removed from the recipe. It should not be altered afterwards. <br />
	 * <br />
	 * Since 3.1 the {@link Prescription} will be added at the end of the list of
	 * the {@link Rezept}, and the same order will be applied on
	 * {@link Rezept#getLines()}.
	 * 
	 * @param p
	 */
	...
	private static class RpZeile {
		...
		public String toString() {
			...
			sb.append(num).append(fieldSeparator).append(name).append(fieldSeparator).append(pck).append(fieldSeparator)
					.append(ds).append(fieldSeparator).append(bem);
			return sb.toString();
		}
