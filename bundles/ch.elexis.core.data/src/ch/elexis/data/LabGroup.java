...
public class LabGroup extends PersistentObject implements Comparable<LabGroup> {
	...
	/**
	 * Creates an new LabGroup with an initial list of entries. Set items to
	 * <code>null</code> toe create an empty LabGroup
	 *
	...
	public void addItem(LabItem item) {
		if (item != null && (item.state() == EXISTS)) {
			...
			if (StringTool.isNothing(exists)) {
				...
				sql.append("INSERT INTO " + GROUP_ITEM_TABLENAME + " (GroupID, ItemID) VALUES (").append(getWrappedId())
						.append(",").append(item.getWrappedId()).append(")");
				j.exec(sql.toString());
			}
		}
	}
	...
	public void removeItem(LabItem item) {
		StringBuilder sql = new StringBuilder();
		sql.append("DELETE FROM " + GROUP_ITEM_TABLENAME + " WHERE GroupID = ").append(getWrappedId()).append(" AND ")
	...
	public void removeAll() {
		StringBuilder sql = new StringBuilder();
		sql.append("DELETE FROM " + GROUP_ITEM_TABLENAME + " WHERE GroupID = ").append(getWrappedId());
		j.exec(sql.toString());
	}
}
