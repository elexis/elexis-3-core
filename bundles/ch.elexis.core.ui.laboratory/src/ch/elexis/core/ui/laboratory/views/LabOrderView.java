...
public class LabOrderView extends ViewPart implements ICallback {
	...
	public boolean createLabOrderPrint(Patient pat, List<LabOrder> labOrders) {
		Brief br = text.createFromTemplateName(Konsultation.getAktuelleKons(), TT_LABORDERS, Brief.LABOR, pat, null);
	...
	/**
	 * Creates an array with {@link LabOrder}s name, ref.value, date/time (empty ->
	 * to be filled out), result (empty -> to be filled out). First line is the
	 * header line.
	 *
	 * @param labOrders     all LabOrders (only those of {@link State.ORDERED} will
	 *                      be considered)
	 * @param patientGender used to load the correct reference value
	 ...
	 *
	 * @param labOrders list of all orders
