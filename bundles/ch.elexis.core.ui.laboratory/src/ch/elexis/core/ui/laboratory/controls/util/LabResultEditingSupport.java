...
public class LabResultEditingSupport extends EditingSupport {
	...
	protected void setUpCellEditor(ColumnViewer viewer) {
		...
		focusCell = new TreeViewerFocusCellManager((TreeViewer) viewer, new FocusCellHighlighter(viewer) {
	...
	private LabResult createResult(LabItem item, Kontakt origin) {
		...
		LabOrder order = new LabOrder(CoreHub.getLoggedInContact(), findMandant(patient), patient, item, null,
				LabOrder.getNextOrderId(), "Eingabe", now);
