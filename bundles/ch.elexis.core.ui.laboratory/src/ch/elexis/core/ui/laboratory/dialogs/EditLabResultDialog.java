...
public class EditLabResultDialog extends TitleAreaDialog {
	...
	public EditLabResultDialog(Shell parentShell, ILabResult act) {
		this(parentShell, LabResult.load(act.getId()));
	}
	...
	private void updateDateTimeModelToTarget() {
		...
		transmissionTime.setTime(date.get(Calendar.HOUR_OF_DAY), date.get(Calendar.MINUTE), date.get(Calendar.SECOND));
		transmissionDate.setDate(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DAY_OF_MONTH));
		...
		date = getAnalyseDate();
		analyseTime.setTime(date.get(Calendar.HOUR_OF_DAY), date.get(Calendar.MINUTE), date.get(Calendar.SECOND));
		analyseDate.setDate(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DAY_OF_MONTH));
	}
	...
	private void updateModelToTarget() {
		if (result != null) {
			if (result.getItem().getTyp() == LabItemTyp.NUMERIC || result.getItem().getTyp() == LabItemTyp.ABSOLUTE) {
