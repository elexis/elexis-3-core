...
/**
 * A class representing Money (as you might have guessed) Simplifies and
 * standardizes calculations with money (rounding, converting to and from
 * strings etc.) The accepted formats of the string representation of an amount
 * depends on the current locale by default the format of can be set to another
 * locale if necessary (globally) The term "Amount" means here always the x.xx
 * form of Money
 *
 * @author gerry
 *
...
public class Money extends Number implements Comparable<Money> {
	...
	/**
	 * Create Money with a specified amount as String This might fail if the string
	 * doesn't conform to the current locale's standard currency format
	 *
	 ...
	 * to the current locale's standard currency format. Note: In Switzerland,
	 * Separator is '.' while in Germany, Austria and Others, it is ','. So if some
	 * Computers in the network have an OS locale setting of Switzerland and others
	 * a german, amounts are entered differently. And even more: The MacOSX does not
	 * use swiss layout at all but sees always ',' as separator. So if Macintosh,
	 * Windows and Linux PC's are in he same network, we must make sure, that the
	 * decimal comma or dot is always honored correctly.
	 *
	 * @param val an amount
