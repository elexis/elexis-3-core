...
public class CBZip2OutputStream extends OutputStream implements BZip2Constants {
	 ...
	 */
	private int[] m_incs = new int[] { 1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524, 88573, 265720, 797161,
			2391484 };
				...
				do {
					...
					for (i = h; i <= 255; i++) {
						...
						while ((m_ftab[((runningOrder[j - h]) + 1) << 8]
								- m_ftab[(runningOrder[j - h]) << 8]) > (m_ftab[((vv) + 1) << 8] - m_ftab[(vv) << 8])) {
	...
	private void qSort3(int loSt, int hiSt, int dSt) {
		...
		while (sp > 0) {
			...
			med = med3(m_block[m_zptr[lo] + d + 1], m_block[m_zptr[hi] + d + 1],
