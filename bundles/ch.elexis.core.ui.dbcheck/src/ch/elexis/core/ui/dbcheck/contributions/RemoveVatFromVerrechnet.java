...
public class RemoveVatFromVerrechnet extends ExternalMaintenance {
	...
	public String executeMaintenance(IProgressMonitor pm, String DBVersion) {
		...
		Display.getDefault().syncExec(() -> {
			MessageDialog dialog = new MessageDialog(Display.getDefault().getActiveShell(), "MWST 0 setzen", null,
					"Für welchen verrechneten Leistungs-Typ soll die MWST 0 gesetzt werden?", MessageDialog.QUESTION, 0,
					"Pandemie", "Tarmed");
		...
		if (StringUtils.isNotBlank(tarifTyp)) {
			...
			sb.append("\nEs wurden " + updateCnt + " Kopien der Leistungen von " + existingCopy.size()
					+ " MWST 0 gesetzt");
