...
public class TableBEHDL_DG_JOINT extends TableDescriptor {

	@Override
	protected String[] returnFieldsIn186() {
		return new String[]{"ID", "lastupdate", "deleted", "BehandlungsID", "DiagnoseID"};
	}

	@Override
	protected String[] returnFieldTypesIn186() {
		return new String[]{"varchar(25)", "bigint(20)", "char(1)", "varchar(25)", "varchar(25)"};
	}
	...
	protected String[] referentialIntegrityCheckSQLin186() {
		return new String[]{
				"BehandlungsID or DiagnoseID do not have an associated entry in BEHANDLUNGEN, DIAGNOSE or are NULL",
				"SELECT * FROM BEHDL_DG_JOINT bdj WHERE bdj.behandlungsid NOT IN (SELECT id FROM BEHANDLUNGEN) OR bdj.diagnoseid NOT IN (SELECT id FROM DIAGNOSEN) OR bdj.behandlungsid IS NULL OR bdj.diagnoseid IS NULL;"};
	}
