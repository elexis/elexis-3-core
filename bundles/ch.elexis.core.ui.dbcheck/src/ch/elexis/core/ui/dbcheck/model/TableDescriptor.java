...
/**
 * Table descriptor of Elexis database model > 1.8.6 see
 * {@link ch.elexis.util.DBUpdate} for current version
 *
...
public abstract class TableDescriptor {
	...
	public String[] getFields(String version) {
		...
		if (version.equalsIgnoreCase(DBModel.VERSION1811) || version.equalsIgnoreCase(DBModel.VERSION1812)
				|| version.equalsIgnoreCase(DBModel.VERSION1813) || version.equalsIgnoreCase(DBModel.VERSION1814)) {
	...
	public String[] getFieldTypes(String version) {
		...
		throw new IllegalStateException("The correct field types entry was not returned for " + version);
	}

	/**
	 * Enthält eine Liste von ungültigen Zuständen innerhalb einer bestimmten
	 * Tabelle in der Form e.g.: Bezeichnung1 LIKE '' die umgebenden Query Teile wie
	 * SELECT FROM etc. müssen selber angehängt werden.
	 *
	 * @param version
	 * @return
	 */
	...
	/**
	 * Enthält eine Liste von Reinigungsskripts einer Tabelle; der ungerade Eintrag
	 * enthält die Beschreibung, der gerade das SQL Statement
	 *
	 * @param version
	 * @return
	 */
	public String[] getCleaningSQLforPostgresSQL(String version) {
		...
		if (version.equalsIgnoreCase(DBModel.VERSION1812))
			return cleaningSQLforPostgreSQLin186();
		if (version.equalsIgnoreCase(DBModel.VERSION1813) || version.equalsIgnoreCase(DBModel.VERSION1814))
			return cleaningSQLforPostgreSQLin186();
		return null;
	}

	/**
	 * Enthält Tests zur referentiellen Integrität. Die SQL Skripts müssen so
	 * definiert sein dass Sie im Fehlerfalle eine Ausgabe erzeugen. Der ungerade
	 * Eintrag enthält die Beschreibung des Fehlers, der gerade das SQL Statement
	 * auf das geprüft wird, und das bei Erzeugen eines Resultates dem angegebenen
	 * Fehler entspricht.
	 *
	 * @param version
	 * @return
