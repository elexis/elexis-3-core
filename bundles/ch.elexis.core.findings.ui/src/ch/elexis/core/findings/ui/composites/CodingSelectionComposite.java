			...
			public void selectionChanged(SelectionChangedEvent event) {
				if (proposalProvider != null) {
					...
					if (selection instanceof StructuredSelection && !selection.isEmpty()) {
						proposalProvider.setSelectedSystem(
								Optional.of((String) ((StructuredSelection) selection).getFirstElement()));
					} else {
		...
		toAddressProposalAdapter.addContentProposalListener(new IContentProposalListener() {
			...
			public void proposalAccepted(IContentProposal proposal) {
				...
				for (Object object : listeners) {
					SelectionChangedEvent selectionEvent = new SelectionChangedEvent(CodingSelectionComposite.this,
							getSelection());
		...
		menuManager.add(new Action("Lokalen Code anlegen") {
			...
			public boolean isEnabled() {
				...
				if (systemSelection instanceof StructuredSelection) {
					...
					if (codeSystem instanceof String
							&& codeSystem.equals(CodingSystem.ELEXIS_LOCAL_CODESYSTEM.getSystem())) {
