...
/**
 * There exists a main group with depth index of 0 all childrens have a depth
 * index > 0
 *
 * @author med1
 *
...
public class CompositeGroup extends Composite implements ICompositeSaveable {
	...
	public CompositeGroup(Composite parent, IFinding iFinding, boolean showTitle, boolean showBorder, int marginWidth,
			int marginTop, int depthIndex) {
		...
		if (depthIndex == 0) {
			...
			this.setToolbarActions(
					FindingsUiUtil.createToolbarMainComponent(titleComposite, (IObservation) iFinding, 1));
		} else {
			if (iFinding instanceof IObservation) {
			...
			if (showTitle && txt != null) {
				...
				if (depthIndex == 1) {
					...
					lbl.setFont(UiDesk.getFont(fontData.getName(), fontData.getHeight() + 3, SWT.BOLD));
				} else if (depthIndex > 1) {
					gridLayout.marginTop = 15;
					gridLayout.marginBottom = 0;
					gdLbl.horizontalIndent = -40;
					lbl.setFont(UiDesk.getFont(fontData.getName(), fontData.getHeight() + 1, SWT.BOLD));
				}
				lbl.setText(txt);
