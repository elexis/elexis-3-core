...
public class MergeLocalCodeDialog extends TitleAreaDialog {
	...
	protected void okPressed() {
		...
		boolean confirm = MessageDialog.openConfirm(getShell(), "Warnung",
	...
	private void mergeLocalCode(ILocalCoding source, ILocalCoding destination) {
		...
		for (IObservation iObservation : obsWithCode) {
			...
			coding = coding.stream()
					.filter(c -> !(c.getSystem().equals(source.getSystem()) && c.getCode().equals(source.getCode())))
		...
		ElexisEventDispatcher.getInstance().fire(new ElexisEvent(null, ICoding.class, ElexisEvent.EVENT_RELOAD));
	}
