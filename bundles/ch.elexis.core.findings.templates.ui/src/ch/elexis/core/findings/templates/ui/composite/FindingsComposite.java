...
public class FindingsComposite extends Composite {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (model.isPresent() && event.getSelection() instanceof StructuredSelection) {
					...
					findingsDetailComposite.setSelection(model.get(),
							element instanceof FindingsTemplate ? (FindingsTemplate) s.getFirstElement() : null);
				}
			}
		});
	}
	...
	private void fillContextMenu(IMenuManager contextMenu, FindingsTemplates findingsTemplates) {
		...
		contextMenu.add(new Action("Neue Vorlage") {
			...
			@Override
			public void run() {
				FindingsDialog findingsDialog = new FindingsDialog(Display.getDefault().getActiveShell(),
						findingsTemplates);
		...
		contextMenu.add(new Action("Vorlagen Exportieren") {
			...
			public void run() {
				...
				if (path != null) {
					try {
						FindingsServiceHolder.findingsTemplateService.exportTemplateToFile(model, path);

						model = FindingsServiceHolder.findingsTemplateService.getFindingsTemplates("Standard Vorlagen");
						setModel(model, true);
					} catch (IOException e) {
		...
		public boolean hasChildren(Object object) {
			if (object instanceof FindingsTemplates) {
				...
				if (findingsTemplate.getInputData() instanceof InputDataGroupComponent) {
					InputDataGroupComponent inputDataGroupComponent = (InputDataGroupComponent) findingsTemplate
							.getInputData();
