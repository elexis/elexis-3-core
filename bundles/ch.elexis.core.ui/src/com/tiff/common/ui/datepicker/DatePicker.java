 ...
 *
 * changes by sebthom ~ renamed private method doubleClick to onMouseDoubleClick
 * ~ renamed private method dateSelected to onDateSelected ~ removed
 * StringBuffer usage in getCurrentMonthName method - corrected
 * firstDayOfTheWeek bug (26.06.2004) ~ setDate will throw the Selection event +
 * you can use setDate(null) to clear the selection, the calendar will display
 * the current date, but getDate will return null until the user explicitely
 * selects a date from the control
 *
 * changes by Daniel Lutz <danlutz@watz.ch> ~ DatePanel.onPaint(): draw
 * currently selected day/date in respect to selectedDate instead of interally
...
public class DatePicker extends Composite {
	...
	private class DatePanel extends Canvas {
		...
		private boolean isSelectedDate(Calendar cal) {
			...
			if (temp.get(Calendar.YEAR) == cal.get(Calendar.YEAR) && temp.get(Calendar.MONTH) == cal.get(Calendar.MONTH)
