...
public class TextTemplatePreferences extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	protected void createFieldEditors() {
		...
		expl.setText(
				Messages.TextTemplatePreferences_ExplanationLine1 + Messages.TextTemplatePreferences_ExplanationLine2
		 ...
		 * = exr.getExtensionPoint("ch.elexis.documentTemplates"); if (exp != null) {
		 * IExtension[] extensions = exp.getExtensions(); for (IExtension ex :
		 * extensions) { IConfigurationElement[] elems = ex.getConfigurationElements();
		 * for (IConfigurationElement el : elems) { String n=el.getAttribute("name");
