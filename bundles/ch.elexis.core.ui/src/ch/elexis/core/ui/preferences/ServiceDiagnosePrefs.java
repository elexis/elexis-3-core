	...
	private void loadInput(ViewType view) {
		...
		switch (view) {
			...
			case Diagnose :
				...
				settings = ConfigServiceHolder
						.getUser(Preferences.USR_SERVICES_DIAGNOSES_DIAGNOSE, getListAsString(aInput)).split(",");
				break;
			case Codes :
				...
				aInput.add(FAVORITES);
				settings = ConfigServiceHolder
						.getUser(Preferences.USR_SERVICES_DIAGNOSES_CODES, getListAsString(aInput)).split(",");
				break;
			default :
				break;
		}
