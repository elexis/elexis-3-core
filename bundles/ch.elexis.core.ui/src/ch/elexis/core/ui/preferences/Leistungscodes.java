...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
	...
	Table table;
	String[] tableCols = {Messages.Leistungscodes_nameOfBillingSystem, Messages.Leistungscodes_billingSystem,
	...
	class AbrechnungsTypDialog_InputDialog extends Dialog {
		 ...
		 *            ListDisplay<String>[], array of ListDisplay's containing item
		 *            names which should not be duplicated
		 * @param text
		 *            String, the initial text value for the text editor field (for the
		 *            item values)
		 ...
		 *            String[], list of types to which this field can be changed to - if
		 *            null, then display no combo
		 */
		...
		protected void calcFieldPresence() {
			...
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.Leistungscodes_dateHL)) {
		...
		protected void okPressed() {
			...
			result[2] = (chNumeric == null || chNumeric.isDisposed())
					? "0" //$NON-NLS-1$
					: ((chNumeric.getSelection()) ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$
			result[3] = (chMultiline == null || chMultiline.isDisposed())
					? "0" //$NON-NLS-1$
					: ((chMultiline.getSelection()) ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$
			result[4] = (chStyled == null || chStyled.isDisposed())
					? "0" //$NON-NLS-1$
	...
	class FieldDefsDisplay extends Composite {
		...
		private FieldDefsDisplay(Composite parent, int style, String[] listItems) {
			...
			listDisplay.addHyperlinks(Messages.Leistungscodes_contactHL, Messages.Leistungscodes_textHL,
					Messages.Leistungscodes_dateHL, Messages.Leistungscodes_comboHL, Messages.Leistungscodes_listHL,
			...
			ListPopUpMenuAction moveItemDownAction = new ListPopUpMenuAction(Messages.Leistungscodes_moveItemDown,
					...
					listDisplay, true);
			ListPopUpMenuAction delItemAction = new ListPopUpMenuAction(Messages.Leistungscodes_deleteAction,
					...
					default :
						if (actionType > LASTFIXEDITEM) {
							moveItemToOtherList(listDisplay,
									moveTo_DestinationLists.get(actionType - LASTFIXEDITEM - 1), noDuplicatesList);
						}
						break;
				}
			}
		...
		public String _FieldsGetLabel(final Object o) {
			...
			if (l.length > 1) {
				...
				String opt = (l.length >= 3)
						? "   (" + l[2].replaceAll(ITEMDELIMITER, "; ") + ")" //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
						: StringTool.leer;
				if (opt.trim().equalsIgnoreCase("(SQL)")) { //$NON-NLS-1$
					opt = (l.length >= 4)
							? "   (SQL: " + l[3].replaceAll(ITEMDELIMITER, "; ") + ")" //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
							: StringTool.leer;
		 ...
		 *            if changing a field, then this contains the full definition for
		 *            the field to be changed [fieldName]:[fieldType]:[itemsList]
		 */
		public void _FieldsHyperlinkActivated(final String l, String fieldToBeChanged) {
			...
			AbrechnungsTypDialog_InputDialog inputDlg = new AbrechnungsTypDialog_InputDialog(getShell(),
					ll + (isChanging ? Messages.Leistungscodes_changeTextInTitleBar : Messages.Leistungscodes_add), msg,
		 ...
		 *            an array of ListDisplay's which contain the values which should
		 *            not be duplicated
		 */
		@SuppressWarnings("unchecked")
