...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
	...
	Table table;
	String[] tableCols = { Messages.AccountListView_name, Messages.Leistungscodes_billingSystem,
	...
	protected Control createContents(final Composite parent) {
		...
		SWTHelper.createHyperlink(ret, Messages.AUF2_new, new HyperlinkAdapter() {
			@Override
		...
		SWTHelper.createHyperlink(ret, Messages.AUF2_delete, new HyperlinkAdapter() {
			@Override
	...
	class AbrechnungsTypDialog_InputDialog extends Dialog {
		...
		protected void calcFieldPresence() {
			...
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.FixMediDisplay_UsageList)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
	...
	class AbrechnungsTypDialog extends TitleAreaDialog {
		...
		protected Control createDialogArea(final Composite parent) {
			...
			new Label(upperPartComp, SWT.NONE).setText(Messages.AccountListView_name);
			...
			new Label(upperPartComp, SWT.NONE).setText(Messages.Leistungscodes_billingSystem);
			...
			// *** label/combo for law
			new Label(upperPartComp, SWT.NONE).setText(Messages.InvoiceListView_tblclmnLaw_text);
			...
			lbTaxp.setText(Messages.Leistungscodes_multiplier);
			...
			fdOpt.setLabel(Messages.FallDetailBlatt2_optionalData);
	...
	class FieldDefsDisplay extends Composite {
		...
		private FieldDefsDisplay(Composite parent, int style, String[] listItems) {
			...
			listDisplay.addHyperlinks(Messages.Leistungscodes_contactHL, Messages.Leistungscodes_textHL,
					Messages.GlobalActions_Redate, Messages.Leistungscodes_comboHL, Messages.FixMediDisplay_UsageList,
		...
		public void _FieldsHyperlinkActivated(final String l, String fieldToBeChanged) {
			...
			if (ll.isEmpty()) {
				...
				if (fieldType.equalsIgnoreCase("K")) { //$NON-NLS-1$
					...
					ll = Messages.FixMediDisplay_UsageList;
			...
			AbrechnungsTypDialog_InputDialog inputDlg = new AbrechnungsTypDialog_InputDialog(getShell(),
					ll + (isChanging ? Messages.AccessControlDefaults_change : Messages.Leistungscodes_add), msg,
			...
			if (inputDlg.open() == Dialog.OK) {
				...
				} else if (ll.equalsIgnoreCase(Messages.FixMediDisplay_UsageList)) {
					if (hasNumeric) {
