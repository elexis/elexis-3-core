...
//						@Override
//						public void run() {
//							Role _role = (Role) tc.getData("role"); //$NON-NLS-1$
//							IRole role = _role.toIRole();
//							ACE ace = (ACE) element;
//							if (ace != null) {
//								boolean hasChildren = !ace.getChildren(false).isEmpty();
//								boolean selfChecked = AccessControlServiceHolder.get().request(role, ace);
//
//								if (selfChecked) {
//									AccessControlServiceHolder.get().revoke(role, ace);
//								} else {
//									AccessControlServiceHolder.get().grant(role, ace);
//								}
//
//								// update check status of all childrens
//								if (hasChildren) {
//									boolean checked = AccessControlServiceHolder.get().request(role, ace);
//									List<ACE> aces = ace.getChildren(true);
//									for (ACE permission : aces) {
//										if (checked) {
//											AccessControlServiceHolder.get().grant(role, permission);
//										} else {
//											AccessControlServiceHolder.get().revoke(role, permission);
//										}
...
//		if (AccessControlServiceHolder.get().request(r.toIRole(), ace.getParent()))
//			return 1;
//
//		List<ACE> chain = ace.getChildren(true);
////		List<Boolean> chainRights = chain.stream().map(ace2 -> AccessControlServiceHolder.get().request(r, ace2))
