	...
	public void doPrint(Patient pat) {
		...
		try {
				...
				public void run(IProgressMonitor monitor) {
					...
					if (tpw.doPrint(actPatient, template, printer, tray, monitor) == false) {
						Status status = new Status(Status.ERROR, "ch.elexis", Status.ERROR, //$NON-NLS-1$
								Messages.Printing_Error_while, null);
						ErrorDialog.openError(null, Messages.Printing_Error_while,
								Messages.TemplateDrucker_docname + template + Messages.KGDrucker_emr,
			...
			ElexisStatus status = new ElexisStatus(ElexisStatus.ERROR, Hub.PLUGIN_ID, ElexisStatus.CODE_NONE,
					Messages.Printing_Error_while + ": " + Messages.TemplateDrucker_couldntOpen, ex); //$NON-NLS-1$
