...
/**
 * Lock multiple elements. By default operation is handled with
 * {@link Strategy#SEQUENTIAL}. The action itself is done in either
 * {@link #doRun(PersistentObject)} or {@link #doRun(List)} depending on
 * strategy.
 *
 * @param <T>
 * @since 3.6
...
public abstract class MultiLockRequestingAction<T extends List<? extends PersistentObject>> extends Action {
	...
	public enum Strategy {
		...
		SEQUENTIAL,
		/**
		 * Lock all elements, calls {@link MultiLockRequestingAction#doRun(List)} if
		 * successful.
		 */
		ALL_OR_NOTHING
	};
	 ...
	 */
	public void acquireLockFailed(LockResponse failedLockResponse, PersistentObject failedPersistentObject) {
	...
	/**
	 * 
	 * @return the objects the lock is requested for, or <code>null</code> to return
	 *         without action
	 */
	...
	/**
	 * The locked objects to perform the action on. Called once on
	 * {@link Strategy#ALL_OR_NOTHING}
	 * 
	 * @param lockedObjects
	 */
