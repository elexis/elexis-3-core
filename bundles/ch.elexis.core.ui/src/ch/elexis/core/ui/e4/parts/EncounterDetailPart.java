...
public class EncounterDetailPart implements IUnlockable {
	
	public static final String ID = EncounterDetailPart.class.getName();
	
	private Logger log = LoggerFactory.getLogger(EncounterDetailPart.class);
	...
	private DiagnosenDisplay diagnosenDisplay;
	private VerrechnungsDisplay verrechnungsDisplay;
	...
	public void postConstruct(Composite parent, EModelService modelService){
		...
		hlMandant.addHyperlinkListener(new HyperlinkAdapter() {
			...
			public void linkActivated(HyperlinkEvent e){
				KontaktSelektor ksl = new KontaktSelektor(Hub.getActiveShell(), Mandant.class,
					Messages.KonsDetailView_SelectMandatorCaption,
		...
		diagnosenDisplay = new DiagnosenDisplay(partService, botleft, SWT.NONE);
		diagnosenDisplay.setLayoutData(SWTHelper.getFillGridData(1, true, 1, true));
		verrechnungsDisplay = new VerrechnungsDisplay(partService, botright, SWT.NONE);
		verrechnungsDisplay.setLayoutData(SWTHelper.getFillGridData(1, true, 1, true));
		
		PartMenus partMenus = new PartMenus();
		partMenus.createViewerContextMenu(menuService, ID,
			verrechnungsDisplay.getViewer().getTable());
		//		getSite().registerContextMenu(ID + ".VerrechnungsDisplay", verrechnungsDisplay.contextMenuManager,
		//			verrechnungsDisplay.viewer);
		//TODO
		//		getSite().setSelectionProvider(verrechnungsDisplay.viewer);
		...
		partMenus.createViewMenu(modelService, mPart, versionDisplayAction, versionFwdAction,
			versionBackAction, GlobalActions.neueKonsAction, GlobalActions.delKonsAction,
		...
		partMenus.createToolbar(modelService, mPart, GlobalActions.neueKonsAction, saveAction);
		...
		ElexisEventDispatcher.getInstance().addListeners(eeli_kons, eeli_kons_sync,
			eeli_user, eeli_fall);
		// TODO
	...
	public void init(MPart mPart){
		...
		//			};
		//		} else {
		String state = mPart.getPersistedState().get(CFG_VERTRELATION);
		if (state == null) {
		...
		state = mPart.getPersistedState().get(CFG_HORIZRELATION);
		if (state == null) {
	...
	private synchronized void setKons(final Konsultation kons){
		...
		if (kons != null) {
			...
			hlMandant.setEnabled(hlMandantEnabled);
			diagnosenDisplay.setDiagnosen(kons);
			verrechnungsDisplay.setLeistungen(kons);
			verrechnungsDisplay.setEnabled(true);
			diagnosenDisplay.setEnabled(true);
			...
			hlMandant.setBackground(hlMandant.getParent().getBackground());
			diagnosenDisplay.clear();
			verrechnungsDisplay.clear();
			text.setText(""); //$NON-NLS-1$
			text.setEnabled(false);
			verrechnungsDisplay.setEnabled(false);
			diagnosenDisplay.setEnabled(false);
		}
		actKons = kons;
		cDesc.layout();
	...
	private void makeActions(){
		...
		versionBackAction = new Action(Messages.KonsDetailView_PreviousEntry) { // $NON-NLS-1$
			...
			public void run(){
				...
				if (MessageDialog.openConfirm(Hub.getActiveShell(),
					Messages.KonsDetailView_ReplaceKonsTextCaption,
					Messages.KonsDetailView_ReplaceKonsTextBody)) {
			...
			public void run(){
				if (actKons != null) {
					AssignStickerDialog asd =
						new AssignStickerDialog(Hub.getActiveShell(), actKons);
					asd.open();
				}
			}
		};
	...
	public void adaptMenus(){
		verrechnungsDisplay.gettVerr().getMenu()
	...
	public void addToVerechnung(Artikel artikel){
		verrechnungsDisplay.addPersistentObject(artikel);
	}
				...
				if (selection instanceof StructuredSelection) {
					if (!selection.isEmpty()) {
						...
						if (!nFall.getId().equals(fallId)) {
							if (!nFall.isOpen()) {
								SWTHelper.alert(Messages.KonsDetailView_CaseClosedCaption,
									Messages.KonsDetailView_CaseClosedBody);
							} else {
								MessageDialog msd = new MessageDialog(Hub.getActiveShell(),
									...
									MessageFormat.format(
										...
										new Object[] {
											...
											Messages.KonsDetailView_Yes, Messages.KonsDetailView_No
									}, 0);
