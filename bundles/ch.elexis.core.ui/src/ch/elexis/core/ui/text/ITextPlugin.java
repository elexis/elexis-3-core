...
public interface ITextPlugin extends IExecutableExtension {
	 ...
	 * 
	 * @param bs
	 *            the byte array with the document in a fotmat the compnent can
	 *            interpret
	 * @param asTemplate
	 ...
	 * 
	 * @param place
	 *            a regular expression in the document, that will be replaced by the
	 *            table. Only the first match will be used.
	 * @param properties
	...
	// TODO make grid lines invisible for tables - Für die Rezept-Tabellen die
	// Linien unsichtbar
	// machen
	// public final static int GRID_VISIBLE = 0x0002; // If set, Grid Lines will be
	// visible
	// Nein, auf 0 setzen geht nicht, mit 1 oder 0 kommt lediglich die RP Erzeugung
	// durcheinander
	 ...
	 * @param x
	 *            ,y,w,h position and size of the rectangle relative to the page
	 *            bounds. Measured as millimeters. The effective position and size
	 *            must match the given values at +- 2mm.
	 * @param text
	 ...
	 * 
	 * @param marke
	 *            regular expression, that describes the insertion point. Only the
	 *            first match will be used, and the inserted Text will replace the
	 *            found string.
	 * @param adjust
	 *            SWT.LEFT oder SWT.RIGHT
	 * @return An implementation specific cursor thar allows a later insert after
	 *         that position
	 */
	 ...
	 * @param toTray
	 *            Name of the tray or null (then tray as defined y the driver or the
	 *            template)
