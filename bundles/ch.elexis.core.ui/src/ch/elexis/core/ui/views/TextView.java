...
public class TextView extends ViewPart implements IActivationListener {
	...
	private void makeActions() {
		...
		loadTemplateAction = new Action(Messages.DocumentSelectDialog_openTemplate) { // $NON-NLS-1$
			@Override
			public void run() {
				DocumentSelectDialog bs = new DocumentSelectDialog(getViewSite().getShell(), CoreHub.actMandant,
		...
		saveTemplateAction = new Action(Messages.TextView_saveAsTemplate) { // $NON-NLS-1$
			...
			public void run() {
				if (actBrief != null) {
					txt.saveTemplate(actBrief.get(Messages.DocumentSelectDialog_subject)); // $NON-NLS-1$
				} else {
		...
		newDocAction = new Action(Messages.TextView_newDocument) { // $NON-NLS-1$
			...
			public void run() {
				...
				if (pat != null) {
					...
					MessageDialog.openInformation(UiDesk.getTopShell(), Messages.AUF2_NoPatientSelected, // $NON-NLS-1$
							Messages.AUF2_NoPatientSelected); // $NON-NLS-1$
				}
			}
