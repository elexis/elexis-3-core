...
public class BestellBlatt extends ViewPart implements ICallback {
	...
	public void createOrder(final IContact receiver, final List<IOrderEntry> toOrder) {
		...
		int i = 1;
		Money sum = new Money();
		tbl[0] = new String[] { Messages.Core_Count, Messages.Core_Phamacode, Messages.Core_Name,
				Messages.BestellBlatt_UnitPrice, Messages.BestellBlatt_LinePrice };
	...
	private void openLocalDocument(BestellBlatt view, Brief brief) {
		...
		if (service != null) {
			...
			if (file.isPresent()) {
				...
				MessageDialog.openError(getSite().getShell(), ch.elexis.core.ui.commands.Messages.Core_Error,
