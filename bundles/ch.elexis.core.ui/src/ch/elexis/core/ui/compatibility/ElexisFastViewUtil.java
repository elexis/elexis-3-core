...
/**
 * since e4 doesnt support fastviews thats we need our own fastview
 * implementation
 *
 * @author med1
 *
...
public class ElexisFastViewUtil {
	 ...
	 * created and the view will be added within a {@link MPlaceholder} element.
	 *
	 * @param perspectiveId
	 * @param viewId
	 */
	public static void addToFastView(String perspectiveId, String viewId) {
		...
		if (window != null) {
			...
			if (obj instanceof MPerspective) {
				...
				MPartStack stack = (MPartStack) eModelService.find(ELEXIS_FASTVIEW_STACK, mPerspective);
				...
				if (stack != null) {
					...
					MToolControl toolControl = (MToolControl) eModelService.find(getToolControlId(window, mPerspective),
							mApplication);

					if (toolControl == null) {
						MTrimBar mTrimBar = eModelService.getTrim(window, SideValue.BOTTOM);
						if (toolControl == null) {
							toolControl = createFastViewToolControl(window, mPerspective, eModelService, mTrimBar);
						}
					}
					if (toolControl != null && !ElexisFastViewUtil.isViewInsideFastview(stack, viewId)
	...
	public static void createFastViewMenuItem(final Menu menu, MPart part) {
		...
		if (mPerspective != null) {
			MPartStack stack = (MPartStack) eModelService.find(ElexisFastViewUtil.ELEXIS_FASTVIEW_STACK, mPerspective);
			...
			menuItemClose.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if (ePartService.savePart(part, true)) {
						...
						MPerspective mPerspective = ElexisFastViewUtil.getService(EModelService.class)
								.getActivePerspective(
										ElexisFastViewUtil.getService(EModelService.class).getTopLevelWindowFor(part));
	...
	private static MToolControl createFastViewToolControl(MTrimmedWindow window, MPerspective mPerspective,
			EModelService eModelService, MTrimBar mTrimBar) {
		if (mTrimBar != null) {
					...
					if (IWorkbenchPage.CHANGE_RESET_COMPLETE.equals(changeId)) {
						UiDesk.asyncExec(new Runnable() {
							...
							public void run() {
								EModelService eModelService = getService(EModelService.class);
								MApplication mApplication = getService(MApplication.class);
								if (eModelService != null && mApplication != null && mApplication.isToBeRendered()) {
						...
						public void run() {
							...
							if (eModelService != null && mApplication != null && mApplication.isToBeRendered()) {
								...
								if (mWindow != null && mWindow.isToBeRendered()) {
									...
									MPerspective mPerspective = eModelService.getActivePerspective(mWindow);
									if (hasFastViewPersistedState(mPerspective)
											&& !hasFastViewPersistedState(mWindow, mPerspective.getElementId())) {
										transferFastViewPersistedState(mPerspective, mWindow);
									}
								}
							}
	...
	private static boolean hasFastViewPersistedState(MTrimmedWindow mWindow, String perspectiveId) {
		EModelService eModelService = getService(EModelService.class);
		Optional<MToolControl> mToolControl = getFastViewToolControl(eModelService, mWindow, perspectiveId,
				SideValue.BOTTOM);
	...
	private static Optional<MPartStack> getFastViewPartStack(MPerspective mPerspective) {
		...
		MPartStack stack = (MPartStack) modelService.find(ElexisFastViewUtil.ELEXIS_FASTVIEW_STACK, mPerspective);
		if (stack != null) {
	...
	/**
	 * Transfer persisted state (incl. size) of fastview {@link MToolControl}
	 * defined in window, to fastview defined in perspective.
	 *
	 * @param fromWindow
	 * @param toPerspective
	 */
	...
	/**
	 * Transfer persisted state (incl. size) of fastview defined in perspective, to
	 * fastview {@link MToolControl} defined in window.
	 *
	...
	public static void transferFastViewPersistedState(MPerspective fromPerspective, MTrimmedWindow toWindow) {
		...
		Optional<MToolControl> mToolControl = getFastViewToolControl(modelService, toWindow, perspectiveId,
				SideValue.BOTTOM);
			...
			mStack.ifPresent(stack -> {
				if (stack.getPersistedState() != null && !stack.getPersistedState().isEmpty()) {
					for (String key : stack.getPersistedState().keySet()) {
						toolControl.getPersistedState().put(key, stack.getPersistedState().get(key));
					}
				}
			});
		});
		...
		if (mWindow != null) {
			...
			if (mPerspective != null) {
				...
				Optional<MToolControl> mToolControl = getFastViewToolControl(eModelService, mWindow, perspectiveId,
						SideValue.BOTTOM);
				if (!mToolControl.isPresent()) {
					mToolControl = getFastViewToolControl(eModelService, mWindow, perspectiveId, SideValue.LEFT);
					mToolControl.ifPresent(toolControl -> {
						MTrimBar trimBarBottom = findTrimBar(eModelService, mWindow, SideValue.BOTTOM);
