...
public class MigratorService implements IMigratorService {
	...
	/**
	 * Migrate the existing personal anamnesis text of a patient to an
	 * {@link IObservation} instance. Migration is only performed if there is not
	 * already a personal anamnesis in form of an {@link IObservation} present for
	 * the patient.
	 *
	 * @param patientId
	 */
		...
		if (patient != null && patient.exists()) {
			...
			if (anamnese != null && !anamnese.isEmpty()) {
				...
				if (observations.isEmpty()) {
					...
					observation
							.setCoding(Collections.singletonList(new TransientCoding(ObservationCode.ANAM_PERSONAL)));
	...
	/**
	 * Migrate the existing risk factors text of a patient to an
	 * {@link IObservation} instance. Migration is only performed if there is not
	 * already a risk factors in form of an {@link IObservation} present for the
	 * patient.
	 *
	 * @param patientId
	 */
	private void migratePatientRiskfactors(String patientId) {
		...
		if (patient != null && patient.exists()) {
			...
			if (risk != null && !risk.isEmpty()) {
				List<IObservation> observations = findingsService.getPatientsFindings(patientId, IObservation.class);
				observations = observations.parallelStream().filter(iFinding -> isRiskfactor(iFinding))
						.collect(Collectors.toList());
				if (observations.isEmpty()) {
					IObservation observation = findingsService.create(IObservation.class);
					observation.setPatientId(patientId);
					...
					observation.setCoding(Collections.singletonList(new TransientCoding(ObservationCode.ANAM_RISK)));
					findingsService.saveFinding(observation);
				}
			}
		}
	...
	/**
	 * Migrate the existing personal animesis text of a patient to an
	 * {@link ICondition} instance. Migration is only performed if there is not
	 * already a diagnose in form of an {@link ICondition} present for the patient.
	 *
	 * @param patientId
	 */
	...
	/**
	 * Migrate the existing family anamnesis text of a patient to an
	 * {@link IFamilyMemberHistory} instance. Migration is only performed if there
	 * is not already a family anamnesis in form of an {@link IFamilyMemberHistory}
	...
	private void migratePatientFamAnamnese(String patientId) {
		...
		if (patient != null && patient.exists()) {
			...
			if (anamnese != null && !anamnese.isEmpty()) {
				List<IFamilyMemberHistory> iFindings = findingsService.getPatientsFindings(patientId,
						IFamilyMemberHistory.class);
				if (iFindings.isEmpty()) {
					IFamilyMemberHistory familyMemberHistory = findingsService.create(IFamilyMemberHistory.class);
	...
	/**
	 * Migrate the existing allergy intolerance text of a patient to an
	 * {@link IAllergyIntolerance} instance. Migration is only performed if there is
	 * not already a allergy intolerance in form of an {@link IAllergyIntolerance}
	 * present for the patient.
	 *
	 * @param patientId
	 */
	private void migrateAllergyIntolerance(String patientId) {
		...
		if (patient != null && patient.exists()) {
			...
			if (allergies != null && !allergies.isEmpty()) {
				List<IAllergyIntolerance> iFindings = findingsService.getPatientsFindings(patientId,
						IAllergyIntolerance.class);
				if (iFindings.isEmpty()) {
					IAllergyIntolerance allergyIntolerance = findingsService.create(IAllergyIntolerance.class);
