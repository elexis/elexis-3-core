...
public class RocketchatMessageTransporter implements IMessageTransporter {
	...
	/**
	 * The webhook integration token to use to send messages as station to
	 * rocketchat.
	 */
	...
	private IStatus sendFromStationSender(TransientMessage message) {
		String integrationToken = configService.getLocal(CFG_ROCKETCHAT_STATION_INTEGRATION_TOKEN, null);
		...
		return new Status(IStatus.ERROR, Bundle.ID, "No webhook integration token ["
				+ CFG_ROCKETCHAT_STATION_INTEGRATION_TOKEN + "] found in local config or malformed url.");
	}
	...
	private IStatus send(URL url, byte[] postDataBytes) throws IOException {
		...
		return new Status(IStatus.ERROR, Bundle.ID, "Error sending, with response code: " + responseCode);
	}
