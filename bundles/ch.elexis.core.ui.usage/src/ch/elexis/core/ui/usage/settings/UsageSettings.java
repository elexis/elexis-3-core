...
public class UsageSettings extends PreferencePage implements IWorkbenchPreferencePage {
	...
	protected Control createContents(Composite parent) {
		...
		TableViewer viewer = new TableViewer(main, SWT.FULL_SELECTION | SWT.BORDER | SWT.SINGLE);
		...
		tvc.setLabelProvider(new ObservableMapCellLabelProvider(
				EMFProperties.value(ModelPackage.Literals.ISTATISTIC__TIME).observeDetail(cp.getKnownElements())) {
			@Override
		...
		tvc.setLabelProvider(new ObservableMapCellLabelProvider(EMFProperties
				.value(ModelPackage.Literals.ISTATISTIC__ACTION_TYPE).observeDetail(cp.getKnownElements())));

		tvc = new TableViewerColumn(viewer, SWT.LEFT);
		tvc.getColumn().setText("Aktion");
		tvc.getColumn().setWidth(300);
		tvc.setLabelProvider(new ObservableMapCellLabelProvider(
				EMFProperties.value(ModelPackage.Literals.ISTATISTIC__ACTION).observeDetail(cp.getKnownElements())));

		tvc = new TableViewerColumn(viewer, SWT.LEFT);
		tvc.getColumn().setText("Wert");
		tvc.getColumn().setWidth(50);
		tvc.setLabelProvider(new ObservableMapCellLabelProvider(
				EMFProperties.value(ModelPackage.Literals.ISTATISTIC__VALUE).observeDetail(cp.getKnownElements())));
		...
		viewer.setInput(EMFProperties.list(ModelPackage.Literals.STATISTICS__STATISTICS)
		...
		checkSentStatistics
				.setText("Nutzungsstatistik aktivieren und beim Beenden übermitteln (Neustart erforderlich)");
