...
public class StickerFilterCommand extends AbstractHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException{
		...
		if (state) {
			...
			StickerSelectionDialog stickerDialog = new StickerSelectionDialog(
				HandlerUtil.getActiveShell(event), "Filtern nach Sticker", IPatient.class);
	...
	private class StickerFilter implements QueryFilter {
		...
		public void apply(IQuery<?> query){
			...
			filterStickers.forEach(
				fs -> ids.addAll(StickerServiceHolder.get()
					.getObjectsWithSticker(fs, IPatient.class).stream().map(IPatient::getId)
