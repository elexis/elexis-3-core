	...
	public SpotlightReadyComposite(Composite parent, int style, EPartService partService,
		...
		listViewerLastPatients.addSelectionChangedListener(event -> {
			...
			if (firstElement != null) {
				((SpotlightShell) getShell()).setSelectedElement(Category.PATIENT.name() + "::" + firstElement[0]);
			} else {
		...
		listLastPatients.addListener(SWT.FocusIn, event -> {
			...
			if (itemCount >= 1) {
				listViewerLastPatients.setSelection(new StructuredSelection(listLastPatients.getItem(0)));
			}
		...
		compositeLeft.setTabList(new Control[]{nextAppointment, listLastPatients});
		...
		CustomLinkWithOptionalImage openReminders = new CustomLinkWithOptionalImage(compositeRight, SWT.NONE,
				Images.IMG_BELL_EXCLAMATION.getImage());
		...
		// listOpenReminders.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
		// false, 1, 1));
		// listOpenReminders.setBackground(getBackground());
		//
		// Label lblNewMessages = new Label(compositeRight, SWT.NONE);
		// lblNewMessages.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
		// 1, 1));
		...
		newDocuments = new CustomLinkWithOptionalImage(compositeRight, SWT.NONE, Images.IMG_DOCUMENT.getImage());
		newDocuments.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		newDocuments.setData(jumpToInboxViewSupplier);

		newLabValues = new CustomLinkWithOptionalImage(compositeRight, SWT.NONE, Images.IMG_VIEW_LABORATORY.getImage());
		newLabValues.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		newLabValues.setData(jumpToInboxViewSupplier);

		compositeRight.setTabList(new Control[]{openReminders, newDocuments, newLabValues});
		...
		setTabList(new Control[]{compositeLeft, compositeRight});
