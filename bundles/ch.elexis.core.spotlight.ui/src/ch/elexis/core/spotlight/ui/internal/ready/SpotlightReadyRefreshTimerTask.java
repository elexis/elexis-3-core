...
public class SpotlightReadyRefreshTimerTask extends TimerTask {
	...
	public SpotlightReadyRefreshTimerTask(IContextService contextService,
		IModelService coreModelService, IAppointmentService appointmentService){
	...
	private void refreshNextAppointment(){
		...
		if (userContact != null) {
			...
			if (userArea != null) {
				IQuery<IAppointment> nextAppointmentQuery =
					coreModelService.getQuery(IAppointment.class, true, false);
				nextAppointmentQuery.and(ModelPackage.Literals.IAPPOINTMENT__SCHEDULE,
					COMPARATOR.EQUALS, userArea.getName());
