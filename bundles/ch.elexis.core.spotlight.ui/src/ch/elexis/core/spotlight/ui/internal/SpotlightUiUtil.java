...
public class SpotlightUiUtil {
	...
	private boolean handleEnter(IEncounter encounter) {
		if (encounter != null) {
			contextService.getRootContext().setNamed(IContextService.SELECTIONFALLBACK, encounter.getPatient());
			contextService.getRootContext().setNamed(IContextService.SELECTIONFALLBACK, encounter);
