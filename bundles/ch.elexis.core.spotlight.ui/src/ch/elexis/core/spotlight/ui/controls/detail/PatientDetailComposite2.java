...
public class PatientDetailComposite2 extends AbstractSpotlightResultEntryDetailComposite
	...
	private CustomLinkWithOptionalImage lastLaboratory;
	 ...
	 * @param style
	 */
	public PatientDetailComposite2(Composite parent, int style){
		super(parent, style);
		...
		lastLaboratory = new CustomLinkWithOptionalImage(this, SWT.NONE, null);
		lastLaboratory.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 1, 1));
		lastLaboratory.getLink().setText("Letztes Labor: -");
		lastLaboratory.setFont(boldDefaultFont);
		
		setTabList(new Control[] {
			balance, listAppointments, lastEncounter, lastLaboratory
		});
		...
		if (patient != null) {
			...
			if (_lastEncounter != null) {
				...
				lastEncounter.getLink()
					.setText("Letzte Konsultation " + util.formatDate(_lastEncounter.getDate().toLocalDate()));
	...
	private void updateLastLaboratory(IPatient patient){
		lastLaboratory.setData(null);
		if (patient != null) {
			...
			if (latestResult != null) {
				lastLaboratory.getLink()
					.setText("Letztes Labor: " + util.formatDate(latestResult.getDate()) + "");
				lastLaboratory.setData("latest-laboratory::" + patient.getId());
				return;
			}
		}
		lastLaboratory.getLink().setText("Letztes Labor: -");
