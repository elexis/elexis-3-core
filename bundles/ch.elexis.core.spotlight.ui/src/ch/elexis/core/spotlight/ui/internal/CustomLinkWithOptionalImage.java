...
/**
 * To represent a selectable read-only widget that contains an element. On selection it shows the
 * selection state and propagates the selection to the spotlight shell.
 *
 */
	...
	public CustomLinkWithOptionalImage(Composite parent, int style, Image image){
		...
		link.addFocusListener(new FocusAdapter() {
			...
			public void focusGained(FocusEvent e){
				link.setBackground(
					parent.getShell().getDisplay().getSystemColor(SWT.COLOR_TITLE_BACKGROUND));
				if (imageLabel != null) {
