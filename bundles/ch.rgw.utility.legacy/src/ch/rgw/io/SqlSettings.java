 ...
 * der jetzigen Version wird nur eine flat table ähnlich wie bei cfgSettings
 * verwendet. Mehrere Anwendungen können dieselbe Datenbank verwenden, wenn sie
 * unterschiedliche Tabellennamen verwenden.
 */
...
public class SqlSettings extends Settings {
	...
	protected void flush_absolute() {
		...
		try {
			...
			sql.append("UPDATE ").append(tbl).append(" SET ").append(valueColumn).append("= ?,")
					.append(LASTUPDATE_COLUMN).append("= ?").append(" WHERE ");
			sql.append(paramColumn).append("= ?");
			if (constraintKey != null && constraintValue != null) {
				sql.append(" AND ").append(constraintKey).append("= ?");
			}
			...
			sql.append("INSERT INTO ").append(tbl).append("(").append(paramColumn).append(",").append(valueColumn)
					.append(",").append(LASTUPDATE_COLUMN);
			if (constraintKey != null && constraintValue != null) {
			...
			while (it.hasNext()) {
				...
				// String
				// sql="SELECT wert FROM "+tbl+" WHERE "+constraint+" AND
				// param="+JdbcLink.wrap(a);
					...
					if (existingValue != null && !existingValue.equals(parameterValue)) {
						if (lastUpdateChanged(parameterName)) {
							logger.warn("Did not flush parameter [" + parameterName
									+ "] because it was changed. Timestamp local (" + getLastUpdate(parameterName)
									+ ") db (" + getSqlLastUpdate(parameterName) + ")");
							continue;
						}
						if (parameterValue == null) {
							...
							// sql=new
							// StringBuilder("DELETE from "+tbl+" WHERE "+constraint+" AND
							// param="+JdbcLink.wrap(a))
						} else {
							...
							// sql=new
							// StringBuilder("UPDATE "+tbl+" SET wert="+JdbcLink.wrap(v)+" WHERE
							// "+constraint+" AND param="+JdbcLink.wrap(a));
						}
					}
				} else {
					if (parameterValue == null) {
	...
	private long getSqlLastUpdate(String param) {
		...
		try {
			...
			StringBuilder sql = new StringBuilder(300);
			sql.append("SELECT ").append(LASTUPDATE_COLUMN).append(" FROM ").append(tbl).append(" WHERE ")
					.append(paramColumn).append("= ?");
			if (constraintKey != null && constraintValue != null) {
				sql.append(" AND ").append(constraintKey).append("= ?");
			}
			selectStatement = j.getPreparedStatement(sql.toString());
