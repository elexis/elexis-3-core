...
public class RocketchatAppender extends AppenderBase<ILoggingEvent> {
	...
	protected void append(ILoggingEvent eventObject){
		try {
			...
			IStatus postStatus =
				new IntegrationPostHandler(eventObject, identification, attachmentBased)
					.post(realIntegrationUrl);
			if (!postStatus.isOK()) {
				addError("Error posting to integrationUrl ["
					+ getContext().getProperty("integrationUrl") + "]: " + postStatus.getMessage());
			}
		} catch (IOException ex) {
			addError("Error posting to integrationUrl ["
				+ getContext().getProperty("integrationUrl") + "]", ex);
		}
	}
