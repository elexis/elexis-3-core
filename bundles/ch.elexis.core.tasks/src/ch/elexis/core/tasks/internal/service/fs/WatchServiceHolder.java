...
public class WatchServiceHolder {
	...
	public WatchServiceHolder(TaskServiceImpl taskServiceImpl) {
		...
		try {
			...
			logger.error("Error instantiating WatchService, filesystem events will not be picked up.", ioe);
		}
		watchService = initWatchService;
	...
	private class WatchServicePoller extends Thread {
				...
				try {
					...
					for (WatchEvent<?> watchEvent : pollEvents) {
						if (taskDescriptor != null) {
							...
							String watcherPath = taskDescriptor.getTriggerParameters()
									.get(IIdentifiedRunnable.RunContextParameter.STRING_URL);
							...
							runContext.put(IIdentifiedRunnable.RunContextParameter.STRING_URL, fullPath);

							trigger(taskDescriptor, runContext);
						} else {
							logger.error("No taskDescriptor registered for the provided watchKey [{}], removing key",
									key);
		...
		private void trigger(ITaskDescriptor taskDescriptor, Map<String, String> runContext) {
			try {
				...
				taskService.trigger(taskDescriptor, null, TaskTriggerType.FILESYSTEM_CHANGE, runContext);
