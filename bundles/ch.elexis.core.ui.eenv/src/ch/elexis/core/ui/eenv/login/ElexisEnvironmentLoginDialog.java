...
public class ElexisEnvironmentLoginDialog extends Dialog {
	...
	/**
	 * url not real served - the request to this url will be catched by the browser
	 * to intercept the code
	 */
	...
	public ElexisEnvironmentLoginDialog(Shell shell, String openidClientSecret, String keycloakUrl,
		...
		try {
			...
			X509EncodedKeySpec keySpecX509 = new X509EncodedKeySpec(Base64.getDecoder().decode(realmPublicKey));
	...
	private void handleExchange(String code) {
		try {
			OpenIdOAuth2AccessToken accessToken = (OpenIdOAuth2AccessToken) oauthService.getAccessToken(code);
			...
			user = new KeycloakUser(CoreModelServiceHolder.get(), preferredUsername, name, familyName, issueTime,
					expirationTime, assignedContactId, roles);
	...
	private Set<String> parseRoles(Jws<Claims> jwsClaim) {
		...
		Map<String, Object> realmAccess = (Map<String, Object>) jwsClaim.getBody().get("realm_access");
		if (realmAccess != null) {
		...
		Map<String, Object> resourceAccess = (Map<String, Object>) jwsClaim.getBody().get("resource_access");
		if (resourceAccess != null) {
			Map<String, Object> elexisRcpOpenidAccess = (Map<String, Object>) resourceAccess.get("elexis-rcp-openid");
			if (elexisRcpOpenidAccess != null) {
				List<String> elexisRcpOpenidAccessRoles = (List<String>) elexisRcpOpenidAccess.get("roles");
	...
	private void validateAssignedContactId(String assignedContactId) {
		...
		if (!assignedContact.isPresent()) {
			throw new IllegalStateException("Invalid assignedContactId [" + assignedContactId + "]");
		}
	}
