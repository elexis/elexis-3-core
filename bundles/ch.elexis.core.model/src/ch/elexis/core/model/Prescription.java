...
public class Prescription extends AbstractIdDeleteModelAdapter<ch.elexis.core.jpa.entities.Prescription>
	...
	public void setPrescriptor(IContact value) {
		if (value != null) {
			getEntityMarkDirty().setPrescriptor(((AbstractIdModelAdapter<Kontakt>) value).getEntity());
		} else {
	...
	public IArticle getArticle() {
		...
		if (article.isEmpty() && StringUtils.isNotBlank(getEntity().getArtikelID())) {
			...
			Optional<IArticle> loadedFallback = CoreModelServiceHolder.get().load(getEntity().getArtikelID(),
					IArticle.class, true);
	...
	public String getStopReason() {
		return StringUtils.defaultString((String) extInfoHandler.getExtInfo(Constants.FLD_EXT_STOP_REASON));
	}
	...
	public String getDisposalComment() {
		return StringUtils.defaultString((String) extInfoHandler.getExtInfo(Constants.FLD_EXT_DISPOSAL_COMMENT));
	}
