...
public class SSDRange {
	...
	/**
	 * Hint what to to if an application does not know this type of Range one of:
	 * prefix:(prefix), append:(append), replace:replace
	 */
	...
	public SSDRange(Element el) {
		...
		if (v != null && v.matches("\\d+,\\d+,\\d+,\\d+")) {
			String[] koor = v.split(",");
			viewport = new Rectangle(Integer.parseInt(koor[0]), Integer.parseInt(koor[1]), Integer.parseInt(koor[2]),
	...
	/**
	 * @since 3.0.0 trimmed down copy of org.eclipse.swt.graphics.Rectangle to
	 *        satisfy dependency
	 */
