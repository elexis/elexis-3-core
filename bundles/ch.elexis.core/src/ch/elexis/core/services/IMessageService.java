...
public interface IMessageService {
	 ...
	 * 
	 * @param sender
	 *            a string which may or may be not resolvable to a user or supported
	 *            scheme
	 * @param recipientUri
	 ...
	 *            {@link #INTERNAL_MESSAGE_URI_SCHEME} to leave the choice to the
	 *            system.<br>
	 *            For external communication use <code>mailto:user@bla.com</code> or
	 *            <code>sms:+4133423</code> (if available). A message has to be
	 *            explicitly marked with
	 *            {@link TransientMessage#setAlllowExternal(boolean)} to use an
