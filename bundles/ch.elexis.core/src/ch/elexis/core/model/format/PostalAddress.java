...
public class PostalAddress {
	...
	private void initFromText(String text) {
		...
		String plzOrt = StringUtils.EMPTY;
		String nameVorname = StringUtils.EMPTY;
		...
		case 3: // NameVorname, Adr1, Ortsangaben ODER Anrede, NameVorname,
			// Ortsangaben
			if (zeileList.get(0).indexOf(StringUtils.SPACE) < 0) {
				// Erste Zeile Anrede
		...
		if (!StringTool.isNothing(nameVorname)) {
			...
			int index = nameVorname.lastIndexOf(StringUtils.SPACE); // Z.B. Von Allmen Christoph
			if (index > 0) {
		...
		if (plzOrt.indexOf(StringUtils.SPACE) > 0) {
			// Read zip code
