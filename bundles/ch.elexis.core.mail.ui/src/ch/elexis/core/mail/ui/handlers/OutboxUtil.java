...
public class OutboxUtil {
	...
	private static Command getCommand() {
		...
		return commandService.getCommand("at.medevit.elexis.outbox.ui.command.getOrCreateElementNoUi");
	}
	...
	public static Object getOrCreateElement(ITaskDescriptor iTaskDescriptor, boolean sent) {
		...
		try {
			...
			ParameterizedCommand parametrizedCommmand = ParameterizedCommand.generateCommand(getCommand(), params);
			return PlatformUI.getWorkbench().getService(IHandlerService.class).executeCommand(parametrizedCommmand,
					null);
		} catch (Exception ex) {
