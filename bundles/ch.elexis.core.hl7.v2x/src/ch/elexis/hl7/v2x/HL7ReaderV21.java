...
public class HL7ReaderV21 extends HL7Reader {
	...
	public ObservationMessage readObservation(HL7PatientResolver patientResolver, boolean createIfNotFound)
		...
		try {
			...
			for (int idx = 0; idx < obsCount; idx++) {
				...
				for (int i = 0; i < oru.getPATIENT_RESULT().getORDER_OBSERVATION(idx).getOBSERVATIONReps(); i++) {
	...
	private void setPatient(ORU_R01 oru, final boolean createIfNotFound) throws ParseException, HL7Exception {
		...
		if (pat == null) {
			...
			// place order number
			String orderNumber = oru.getPATIENT_RESULT().getORDER_OBSERVATION().getORC().getOrc2_PLACERORDER()
			...
			if ((patid == null) || (list.size() != 1)) {
				...
				// name and birthdate
				list = patientResolver.findPatientByNameAndBirthdate(StringTool.normalizeCase(lastName),
						StringTool.normalizeCase(firstName), new TimeTool(birthDate).toString(TimeTool.DATE_COMPACT));
