...
public class HL7ReaderV231 extends HL7Reader {
	...
	public ObservationMessage readObservation(HL7PatientResolver patientResolver, boolean createIfNotFound)
		...
		try {
			...
			int obsCount = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getORCOBRNTEOBXNTECTIReps();
			for (int idx = 0; idx < obsCount; idx++) {
				OBR obr = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getORCOBRNTEOBXNTECTI(idx).getOBR();
				...
				for (int i = 0; i < oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getORCOBRNTEOBXNTECTI(idx)
						.getOBXNTEReps(); i++) {
					ORU_R01_ORCOBRNTEOBXNTECTI obs = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI()
							.getORCOBRNTEOBXNTECTI(idx);
					...
					// result
					readOBXResults(obs.getOBXNTE(i), commentNTE, group, sequence, obrObservationDateTime);
				}
			}
	...
	private void setPatient(ORU_R01 oru, final boolean createIfNotFound) throws ParseException, HL7Exception {
		...
		if (pat == null) {
			...
			PID pid = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getPIDPD1NK1NTEPV1PV2().getPID();
			...
			// place order number
			String orderNumber = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getORCOBRNTEOBXNTECTI().getORC()
			...
			if (familyLastName.getFn1_FamilyName().getValue() != null
				...
				lastName = pid.getPid5_PatientName(0).getFamilyLastName().getFn2_LastNamePrefix().getValue()
						+ StringUtils.SPACE
	...
	private void setOrderComment(ORU_R01 oru, int idx, String obsDate) throws ParseException {
		String orderCommentNTE = getComments(oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI().getORCOBRNTEOBXNTECTI(idx),
				-1);
	...
	private void readOBXResults(ORU_R01_OBXNTE obs, String commentNTE, String group, String sequence,
	...
	public OrcMessage getOrcMessage() {
		try {
			ORU_R01 oru = (ORU_R01) message;
			if (oru != null) {
				ORU_R01_PIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI pr = oru.getPIDPD1NK1NTEPV1PV2ORCOBRNTEOBXNTECTI();
				if (pr != null) {
