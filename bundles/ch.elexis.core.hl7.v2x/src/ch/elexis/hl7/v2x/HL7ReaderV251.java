...
public class HL7ReaderV251 extends HL7Reader {
	...
	private void readObservationOulR22(boolean createIfNotFound) throws ParseException, HL7Exception {
		OUL_R22 oul = (OUL_R22) message;
		OUL_R22_ORDER order = oul.getSPECIMEN().getORDER();
		msh = oul.getMSH();

		PID pid = oul.getPATIENT().getPID();
		String orderNumber = order.getOBR().getObr2_PlacerOrderNumber().getEi1_EntityIdentifier().getValue();
