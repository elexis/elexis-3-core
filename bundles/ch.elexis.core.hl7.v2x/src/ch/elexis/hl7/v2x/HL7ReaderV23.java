...
public class HL7ReaderV23 extends HL7Reader {
		...
		try {
			...
			for (int idx = 0; idx < obsCount; idx++) {
				...
				if (obrObservationDateTime == null || obrObservationDateTime.length() < 8) {
					obrObservationDateTime = HL7Helper.dateToString(observation.getDateTimeOfMessage());
				}

				setOrderComment(oru, idx, obrObservationDateTime);

				for (int i = 0; i < oru.getRESPONSE().getORDER_OBSERVATION(idx).getOBSERVATIONReps(); i++) {
					...
					// result
					readOBXResults(obs.getOBSERVATION(i), commentNTE, group, sequence, obrObservationDateTime);
				}
			}
	...
	private void setPatient(ORU_R01 oru, final boolean createIfNotFound) throws ParseException, HL7Exception {
		...
		if (pat == null) {
			...
			// place order number
			String orderNumber = oru.getRESPONSE().getORDER_OBSERVATION().getORC().getOrc2_PlacerOrderNumber(0)
			...
			String patientNotesAndComments = readPatientNotesAndComments(oru.getRESPONSE().getPATIENT());
	...
	private void readOBXResults(ORU_R01_OBSERVATION obs, String commentNTE, String group, String sequence,
		...
		if (valueType.equals(HL7Constants.OBX_VALUE_TYPE_ED)) {
			...
			if (tmp instanceof ST) {
				...
				if (((FT) tmp).getExtraComponents() != null && ((FT) tmp).getExtraComponents().numComponents() > 0) {
