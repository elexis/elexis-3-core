...
public class HL7_ORU_R01 extends HL7Writer {
	 ...
	 *
	 * @param sendingApplication1    Value for MSH3.1
	 * @param sendingApplication3    Value for MSH3.3
	 * @param receivingApplication1  Value for MSH5.1
	 * @param receivingApplication3  Value for MSH5.3
	 * @param receivingFacility      Value for MSH6.1
	 * @param uniqueMessageControlID Value for MSH10
	 * @param uniqueProcessingID     Value for MSH11
	 * @param mandant                Value for MSH4
	 */
	 ...
	 * @param readWithValidation True for parsing with validation, False for parsing
	 *                           without validation
	...
	public ORU_R01 read(String text, boolean readWithValidation) throws HL7Exception {
		...
		if (hl7Msg instanceof ORU_R01) {
			...
			addError(MessageFormat.format(Messages.HL7_ORU_R01_Error_WrongMsgType, hl7Msg.getName()));
		}
		return null;
	}
	 ...
	 */
	public ObservationMessage readObservation(final String text) throws IOException, ElexisException {
	...
	public ObservationMessage readObservation(final String text, boolean readWithValidation)
		...
		try {
			...
			if (oru != null) {
				...
				if ((pid2_patientId == null) || (StringUtils.EMPTY.equals(pid2_patientId)))
				...
				String pid5_patientLastName = StringUtils.EMPTY;
				String pid5_patientFirstName = StringUtils.EMPTY;
				...
				FT[] nteAfterPid_patientNotesAndCommentsArray = oru.getPATIENT_RESULT().getPATIENT().getNTE()
						.getComment();
				String nteAfterPid_patientNotesAndComments = String.join(StringUtils.LF,
				...
				String orc3_fillerOrderNumber = oru.getPATIENT_RESULT().getORDER_OBSERVATION().getORC()
						.getOrc3_FillerOrderNumber().getCm_filler1_UniqueFillerId().getValue();
				String orc9_dateTimeOfTransaction = oru.getPATIENT_RESULT().getORDER_OBSERVATION().getORC()
						.getOrc9_DateTimeOfTransaction().getComponent(0).toString();
				observation = new ObservationMessage(msh3_sendingApplication, msh4_sendingFacility,
						msh7_dateTimeOfMessage, msh10_messageControlId, orc9_dateTimeOfTransaction, pid2_patientId,
						pid5_patientLastName, pid5_patientFirstName, nteAfterPid_patientNotesAndComments,
						pid7_patientBirthdate, pid8_patientSex, pid4_alternatePatientId, orc2_placerOrderNumber,
				...
				for (int j = 0; j < obscount; j++) {
					...
					if ((obrDateOfObservation == null) || (StringUtils.EMPTY.equals(obrDateOfObservation)))
						obrDateOfObservation = obr.getObr22_ResultsReportStatusChangeDateTime().getComponent(0)
								.toString();
					...
					for (int i = 0; i < oru.getPATIENT_RESULT().getORDER_OBSERVATION(j).getOBSERVATIONReps(); i++) {
						...
						if (HL7Constants.OBX_VALUE_TYPE_ED.equals(valueType)) {
							...
							if (!"DOCUMENT".equals(observationId)) { //$NON-NLS-1$
								addWarning(MessageFormat.format(Messages.HL7_ORU_R01_Error_WrongObsIdentifier,
										observationId));
							}
							...
							String dateOfObservation = obx.getObx14_DateTimeOfTheObservation().getComponent(0)
									.toString();
							EncapsulatedData encapsData = new EncapsulatedData(filename, encoding, data,
									dateOfObservation, commentNTE, null, null);
							observation.add(encapsData);
						} else if (HL7Constants.OBX_VALUE_TYPE_ST.equals(valueType)) {
							String kuerzel = obx.getObx3_ObservationIdentifier().getCe1_Identifier().getValue();
							String name = obx.getObx3_ObservationIdentifier().getCe2_Text().getValue();
							...
							String dateOfObservation = obx.getObx14_DateTimeOfTheObservation().getComponent(0)
									.toString();
							StringData strData = new StringData(name, unit, valueST, range, dateOfObservation,
									commentNTE, null, null);
							...
							if (value instanceof ST) {
								...
								addError(MessageFormat.format("Value type of FT ({0}) is not implemented!", //$NON-NLS-1$
							...
							String dateOfObservation = obx.getObx14_DateTimeOfTheObservation().getComponent(0)
									.toString();
							StringData data = new StringData(name, unit, valueST, range, dateOfObservation, commentNTE,
									null, null);
