...
public class DocumentSendToCancerRegister extends AbstractHandler implements IHandler {
	...
	private Optional<IMandator> getAuthor(IDocument iDocument) {
		if (iDocument.getAuthor() != null && iDocument.getAuthor().isMandator()) {
			Optional<IMandator> ret = CoreModelServiceHolder.get().load(iDocument.getAuthor().getId(), IMandator.class);
			if (ret.isPresent()) {
				return ret;
			}
		} else {
			KontaktSelektor ksl = new KontaktSelektor(Display.getDefault().getActiveShell(), Mandant.class,
					"Mandant Auswahl",
			...
			if (ksl.open() == Window.OK) {
				...
				if (mandator != null) {
					Optional<IMandator> mandatorContact = CoreModelServiceHolder.get().load(mandator.getId(),
							IMandator.class);
