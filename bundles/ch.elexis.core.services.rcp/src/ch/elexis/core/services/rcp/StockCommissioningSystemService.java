...
import ch.elexis.core.services.rcp.internal.Bundle;

/**
 * A service for performing article outlays via a stock management system. This
 * implementation currently one sends the respective events to the Elexis
 * server, which handles the connection to the stock management system.
 */
@Component
...
//		return StockCommissioningSystemDriverFactories.getAllDriverUuids();
	...
	public String getInfoStringForDriver(UUID driverUuid, boolean extended) {
		return "not supported on Elexis RCP";
//		return StockCommissioningSystemDriverFactories.getInfoStringForDriver(driverUuid, extended);
	...
	public IStatus performArticleOutlay(IStockEntry stockEntry, int quantity, Map<String, Object> data) {
		...
		performOutlayEvent.getProperties().put(ElexisEventTopics.STOCK_COMMISSIONING_PROPKEY_STOCKENTRY_ID,
				stockEntry.getId());
		performOutlayEvent.getProperties().put(ElexisEventTopics.STOCK_COMMISSIONING_PROPKEY_QUANTITY,
	...
	public IStatus synchronizeInventory(IStock stock, List<String> articleIds, Map<String, Object> data) {
		...
		synchronizeEvent.getProperties().put(ElexisEventTopics.STOCK_COMMISSIONING_PROPKEY_STOCK_ID, stock.getId());
