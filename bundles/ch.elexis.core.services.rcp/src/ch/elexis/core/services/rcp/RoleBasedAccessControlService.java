...
public class RoleBasedAccessControlService implements IAccessControlService {
	...
	public boolean evaluate(EvaluatableACE evaluatableAce) {
		...
		if (activeUserId != null) {
			...
			boolean result = evaluateACE(combinedRolesAclMap.get(combinedRolesHashCode), evaluatableAce);
	...
	public AccessControlList getOrLoadRoleAccessControlList(String roleId) {
		if (!roleAclMap.containsKey(roleId)) {
			...
			if (ElexisSystemPropertyConstants.IS_EE_DEPENDENT_OPERATION_MODE) {
				...
				doPrivileged(() -> {
					IRole role = PortableServiceLoader.getCoreModelService().load(roleId, IRole.class).orElseThrow();
	...
	private boolean evaluateAobo(List<String> aoboMandatorIds, ObjectEvaluatableACE _ace) {
		Optional<Identifiable> object = PortableServiceLoader.get(IStoreToStringService.class)
				.loadFromString(_ace.getStoreToString());
		if (object.isPresent()) {
			String id = null;
	...
	public List<String> getAoboMandatorIds() {
		if (userService == null) {
			userService = PortableServiceLoader.get(IUserService.class);
		}
