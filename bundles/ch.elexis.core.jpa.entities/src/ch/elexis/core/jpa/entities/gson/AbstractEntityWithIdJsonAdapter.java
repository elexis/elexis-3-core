...
/**
 * JSON De-/Serialize an {@link AbstractEntityWithId} with its ID only. Used by Gson. Applied as
 * annotation to a field in a JPA entity.
 */
public class AbstractEntityWithIdJsonAdapter extends TypeAdapter<AbstractEntityWithId> {
	 ...
	 * @return
	 * @throws IOException
	 *             if id can not be loaded or class for entityType can not be found
	 */
	private AbstractEntityWithId tryToLoad(String entityType, String id) throws IOException{
		if (StringUtils.isNotBlank(entityType) && StringUtils.isNotBlank(id)) {
			...
			try {
				...
				EntityManager em = (EntityManager) ElexisEntityManagerServiceHolder
					.getEntityManager().getEntityManager(true);
				try {
					AbstractEntityWithId entity = (AbstractEntityWithId) em.find(clazz, id);
					if (entity == null) {
						throw new IOException(
							"entityType [" + entityType + "] with id [" + id + "] not found");
					}
					return entity;
				} finally {
