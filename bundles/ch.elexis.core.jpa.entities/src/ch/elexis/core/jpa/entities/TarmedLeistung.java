...
@NamedQueries({
		...
		@NamedQuery(name = "TarmedLeistung.title.validUntil", query = "SELECT tl FROM TarmedLeistung tl WHERE tl.deleted = false AND tl.tx255 LIKE :title AND tl.gueltigBis = :validUntil ORDER BY tl.code_ ASC"),
...
public class TarmedLeistung extends AbstractEntityWithId implements EntityWithId, EntityWithDeleted {
	...
	// TarmedLeistung} for the provided
	// * {@link Mandant}.
	// *
	// * @param mandant
	// * @return
	...
	// List<String> ret = new ArrayList<>();
	// List<String> groups =
	// getExtStringListField(TarmedLeistung.EXT_FLD_SERVICE_GROUPS);
	// if (!groups.isEmpty()) {
	...
	// List<String> ret = new ArrayList<>();
	// List<String> blocks =
	// getExtStringListField(TarmedLeistung.EXT_FLD_SERVICE_BLOCKS);
	// if (!blocks.isEmpty()) {
	...
	// getExtStringListField(TarmedLeistung.EXT_FLD_HIERARCHY_SLAVES);
