...
public abstract class AbstractHL7PatientResolver extends HL7PatientResolver {
	...
	public IPatient createPatient(String lastName, String firstName, String birthDate, String sex) {
		...
		return new IContactBuilder.PatientBuilder(PortableServiceLoader.getCoreModelService(), firstName, lastName,
	...
	public List<? extends IPatient> getPatientById(String patid) {
		INamedQuery<IPatient> namedQuery = PortableServiceLoader.getCoreModelService().getNamedQuery(IPatient.class,
				"code");
		return namedQuery.executeWithParameters(namedQuery.getParameterMap("code", StringTool.normalizeCase(patid)));
	}

	@Override
	public List<? extends IPatient> findPatientByNameAndBirthdate(String lastName, String firstName, String birthDate) {
		IQuery<IPatient> patientQuery = PortableServiceLoader.getCoreModelService().getQuery(IPatient.class);
		patientQuery.and(ModelPackage.Literals.IPERSON__LAST_NAME, COMPARATOR.EQUALS, lastName, true);
		patientQuery.and(ModelPackage.Literals.IPERSON__FIRST_NAME, COMPARATOR.EQUALS, firstName, true);
