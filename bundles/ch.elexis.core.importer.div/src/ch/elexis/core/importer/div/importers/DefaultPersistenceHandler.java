...
public class DefaultPersistenceHandler implements IPersistenceHandler {
	...
	public List<ILabOrder> getLabOrdersByOrderId(String orderId) {
		IQuery<ILabOrder> query = PortableServiceLoader.getCoreModelService().getQuery(ILabOrder.class);
	...
	public IPatient loadPatient(String id) {
		Optional<IPatient> loaded = PortableServiceLoader.getCoreModelService().load(id, IPatient.class);
