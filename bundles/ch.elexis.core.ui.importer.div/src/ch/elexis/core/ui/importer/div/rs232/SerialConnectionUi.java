...
public class SerialConnectionUi {
	...
	public static Thread awaitFrame(Connection connection, final Shell shell, final String text,
		final int timeout, final boolean background, final boolean adjustEndTime){
		...
		public void run(){
			final IRunnableWithProgress runnableWithProgress = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException,
					...
					while (!monitor.isCanceled() && System.currentTimeMillis() < endTime
						&& connection.isOpen()) {
						if (count == 160) {
