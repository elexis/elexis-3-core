...
public class KontaktImporterBlatt extends Composite {
	...
	public boolean importKK(final String file) {
		...
		exw.setFieldTypes(new Class[] { Integer.class, String.class, String.class, String.class, String.class,
		...
		for (int i = exw.getFirstRow() + 1; i <= exw.getLastRow(); i++) {
			...
			Organisation kk = KontaktMatcher.findOrganisation(name, null, adr[0], adr[1], adr[2], CreateMode.CREATE);
			if (kk == null) {
				return false;
			}
	...
	public boolean importXML(final String file) {
		// Please keep in sync with doc/import.textile !!
		SWTHelper.showError(Messages.KontaktImporterBlatt_DatatypeErrorHeading,
				Messages.KontaktImporterBlatt_DatatypeErrorText, Messages.KontaktImporterBlatt_xmlImportNotSupported);
		return false;
	}
	...
	public boolean importCSV(final String file) {
		...
		if (mediportInsuererList) {
			...
			try {
				...
				// read csv file
				List<Organisation> importedInsurer = MediportInsurerImporter
						.importCSVFromStream(new FileInputStream(file));
				SWTHelper.showInfo(Messages.KontaktImporterBlatt_csvImportMediportInsurerDone, MessageFormat
