...
/**
 * A class to import any data from different sources. The user can choose the
 * type of data and how to map the fields. He can specify a key file for mapping
 * the data to existing objects.
 *
...
public class GenericImporterBlatt extends Composite {
	...
	static final String[] METHODS = new String[] { "XLS" //$NON-NLS-1$
	};
	...
	class GenericImporterWizard extends Wizard {
		...
		class MappingPage extends WizardPage {
			...
			MappingPage() {
				...
				String description = Messages.GenericImporterBlatt_PleaseSelectType
			...
			/**
			 * Set the requested object type. Intialize available fields according to the
			 * type.
			 *
			...
			/**
			 * Check if the mapping is complete - Fields are chosen - There are the same
			 * size of input and db fields chosen - There is at least one key field defined
			 */
		...
		class SyncPage extends WizardPage {
			...
			public void createControl(Composite parent) {
				...
				new Label(composite, SWT.NONE).setText(Messages.GenericImporterBlatt_ImportAllDataNew);
				...
				// import new elements button
				new Label(composite, SWT.NONE).setText(Messages.GenericImporterBlatt_UpdateSelectedData);
			...
			private void sync() {
				...
				/*
				 * bad performance. we actually don't require to load all objects from the
				 * database Query<PersistentObject> query = new
				 * Query<PersistentObject>(template.getClass()); List<PersistentObject>
				 * dbObjects = query.execute(); if (dbObjects == null) { dbObjects = new
				 * ArrayList<PersistentObject>(); }
				 */
			 ...
			 * List<PersistentObject> dbObjects, List<KeyFields> keyFields) { for
			 * (PersistentObject dbObject : dbObjects) { boolean found = true; for
			 * (KeyFields keyField : keyFields) { String inputValue =
			 * inputObject.get(keyField.inputName); String dbValue =
			 * dbObject.get(keyField.dbName); if (dbValue == null ||
			 * !dbValue.equals(inputValue)) { found = false; break; } } if (found) { return
			 * dbObject; } }
			 *
			...
			/**
			 * Update a single SyncElement to the database. Remove the element form the
			 * current list (since its now of type EQUAL).
			 *
