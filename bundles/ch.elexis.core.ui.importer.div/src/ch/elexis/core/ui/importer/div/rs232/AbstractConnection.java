...
public abstract class AbstractConnection implements PortEventListener {
	...
	public synchronized void awaitFrame(final Shell shell, final String text, final int start, final int end,
			final int following, final int timeout, final boolean background, final boolean adjustEndTime) {
		frameStart = start;
		frameEnd = end;
		overhang = following;
	 ...
	 * the port buffer is read until it is drained, when no more data is available
	...
	public abstract void serialEvent(final int state, final InputStream inputStream, final SerialPortEvent e)
			throws IOException;
