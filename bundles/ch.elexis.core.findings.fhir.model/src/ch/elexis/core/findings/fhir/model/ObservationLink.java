...
public class ObservationLink extends AbstractIdModelAdapter<ch.elexis.core.jpa.entities.ObservationLink>
		implements
			IObservationLink {
	...
	public Optional<IObservation> getSource() {
		return Optional.ofNullable(FindingsModelService.getAdapter(getEntity().getSource(), IObservation.class));
	}

	@Override
	public Optional<IObservation> getTarget() {
		return Optional.ofNullable(FindingsModelService.getAdapter(getEntity().getTarget(), IObservation.class));
	}
	...
	public void setTarget(IObservation observation) {
		if (observation != null) {
			getEntity().setTarget(
					FindingsModelService.getDBObject(observation, ch.elexis.core.jpa.entities.Observation.class));
		} else {
