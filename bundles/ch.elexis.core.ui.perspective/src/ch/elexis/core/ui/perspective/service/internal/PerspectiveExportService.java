...
public class PerspectiveExportService implements IPerspectiveExportService {
	...
	/**
	 * Workaround for https://bugs.eclipse.org/bugs/show_bug.cgi?id=481996 Set the
	 * references of the placeholder to the corresponding parts again in the copy.
	 */
	private static MPerspective clonePerspectiveWithWorkaround(EModelService modelService, MPerspective original) {
		...
		// For each placeholder in the new perspective, set the reference value to the
		// one from the old perspective
		for (MPlaceholder placeholderClone : placeholderClones) {
			// Search for the corresponding placeholder in the "old" perspective
			List<MPlaceholder> placeholderOriginal = modelService.findElements(original,
					placeholderClone.getElementId(), MPlaceholder.class, null, EModelService.IN_ANY_PERSPECTIVE);
			if (placeholderOriginal.size() == 1) {
				// We found only one corresponding placeholder element. Set reference of old
